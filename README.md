...
## v2.0.8 Argon2id 密码迁移

- 新安装、修改密码和重置密码统一使用 Argon2id：32 MiB 临时内存、3 轮计算、并行度 1、24 字节随机盐和 32 字节摘要。
- 完整兼容 v2.0.7 的 PBKDF2-HMAC-SHA-512 与更早的 PBKDF2-HMAC-SHA-256，升级后不会导致旧账号无法登录。
- 旧格式登录成功后只提示迁移，不再后台静默改写；管理员确认当前密码后才原地替换校验值，登录密码本身不变。
- 如果旧密码不符合新版强度规则，仍允许在验证正确后迁移；只有设置新密码时才执行当前强度策略。
- 登录后的升级提示每个会话只弹出一次；稍后可在“设置”页面手动完成。
- Argon2id 内存仅在登录、二次验证、修改密码或迁移时短暂占用，空闲时不会常驻增加 32 MiB；同时最多执行 2 次派生，默认峰值工作内存约 64 MiB。
...
- Passkey 登录、添加与凭据列表统一改用简洁的环形密钥符号，移除视觉过密的指纹图形。
- 所有 `ghost` 操作按钮恢复稳定边框、底色与阴影，默认状态即可识别为按钮，不再依赖点击后的 iOS 粘滞 hover 才出现按钮块。
- 文件收藏、更多操作、弹窗关闭和用户列表补齐默认交互边界，避免可点击文字与普通文本混淆。
- Passkey、SSH 密钥与凭据行改为稳定网格布局；手机端危险操作自动独占整行，减少挤压和误触。
- 小屏含操作区的标题改为纵向排布，修复多个按钮与标题说明相互挤压的问题。
- 新增 UI 回归脚本，直接校验按钮默认块、Passkey 新图标、凭据布局和横向溢出，防止测试未加载最终样式却误报通过。
- 静态资源查询参数升级到 v2.0.6，确保升级后浏览器不会继续复用旧版 CSS 缓存。
...
- v2.0.5 当时使用 PBKDF2-HMAC-SHA-512；v2.0.8 已改为 Argon2id，并保留 PBKDF2 只读兼容验证。
...
- 密码使用 Argon2id（32 MiB、3 轮、并行度 1、随机盐）保存；旧 PBKDF2 配置在明确确认后迁移
...
LukePanel v2.0.6 的页面、路由、状态、API 调用和弹窗均使用 React 18 + TypeScript。React 与 ReactDOM 固定版本随仓库内嵌，生产运行不依赖 CDN；旧版手写 DOM 与过渡前端均不进入正式资源链路。

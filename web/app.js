...
function githubPage(){
  ...
  const latest=data?.latest_release,tagSuggestion=latest?.tag_name?nextVersionSuggestion(latest.tag_name):'v0.9.9-beta'
...
function nextVersionSuggestion(current){const match=String(current).match(/^v(\d+)\.(\d+)\.(\d+)(.*)$/);if(!match)return 'v0.9.9-beta';const major=Number(match[1]),minor=Number(match[2]),patch=Number(match[3]),suffix=match[4]||'';if(major===0&&minor<9)return 'v0.9.9-beta';if(major===0&&minor===9)return `v0.9.${patch+1}${suffix||'-beta'}`;return `v${major}.${minor+1}.0`}

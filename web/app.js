...
function rememberRoute(pathname) { if (knownRoutes.has(pathname))
    sessionStorage.setItem(ROUTE_STORAGE_KEY, pathname); }
...
async function secureApi(url, options = {}) {
    try {
        ...
        if (error.status !== 403 || !String(error.message).includes('二次验证'))
            throw error;
...
function navigate(path, { replace = false } = {}) {
    ...
    if (location.pathname !== target) {
        replace ? history.replaceState({}, '', target) : history.pushState({}, '', target);
    }
...
document.addEventListener('click', e => { const link = e.target.closest('[data-nav]'); if (link) {
    e.preventDefault();
...
document.addEventListener('visibilitychange', () => {
    syncOverviewUpdates();
    syncDockerStats();
    if (!document.hidden && location.pathname === '/' && state.authenticated)
        loadOverview(true);
});
async function restore() {
    ...
    try {
        ...
        state.sessionID = me.session_id;
        state.settings = await api('/api/v1/settings');
        const navigationType = performance.getEntriesByType?.('navigation')?.[0]?.type;
        const saved = rememberedRoute(requestedPath);
        if (location.pathname === '/' && navigationType === 'reload' && saved !== '/')
            history.replaceState({}, '', saved);
        else if (!knownRoutes.has(location.pathname))
            history.replaceState({}, '', requestedPath);
...
function navItems() {
    ...
    if (githubHelperEnabled())
        items.push(['GitHub 助手', '/github', 'github']);
...
function shell(content) {
    const nav = navItems(), mobileRoutes = new Set(['/', '/system', '/docker', '/tools', '/security']), showMobileNav = mobileRoutes.has(location.pathname);
    ...
    return `<div class="app-shell product-shell ${showMobileNav ? 'has-mobile-nav' : 'no-mobile-nav'}"><aside class="sidebar product-sidebar"><div class="brand">${brandIcon()}<div><strong>LukePanel</strong><span>${escapeHTML(state.settings?.version || '服务器管理面板')}</span></div></div><nav class="sidebar-nav">${nav.map(([label, href, i]) => `<a data-nav href="${href}" class="${isActive(href) ? 'active' : ''}">${icon(i, 18)}<span>${escapeHTML(label)}</span></a>`).join('')}</nav><div class="sidebar-footer"><div class="sidebar-user-card"><span class="user-avatar">${escapeHTML((state.username || 'L').slice(0, 1).toUpperCase())}</span><div><strong>${escapeHTML(state.username || '管理员')}</strong><small>本机管理员</small></div></div><button id="theme-toggle" class="icon-text-button">${icon(theme() === 'dark' ? 'sun' : 'moon', 18)}<span>${theme() === 'dark' ? '浅色模式' : '深色模式'}</span></button><button data-logout class="icon-text-button danger-text">${icon('logout', 18)}<span>退出登录</span></button></div></aside><main class="main-content">${content}</main>${mobileNav}${modalHTML()}</div>`;
}
function pageHeader(title, description = '', actions = '') {
    const parent = routeParents[location.pathname];
    return `<header class="page-header product-page-header"><div class="page-header__main">${parent ? `<button class="page-back" data-back="${parent}" aria-label="返回">${icon('back', 20)}</button>` : ''}<div class="page-header__copy"><h1>${escapeHTML(title)}</h1>${description ? `<p>${escapeHTML(description)}</p>` : ''}</div></div>${actions ? `<div class="page-header__actions">${actions}</div>` : ''}</header>`;
}
...
function formatBytes(value) { let v = Number(value || 0); const u = ['B', 'KB', 'MB', 'GB', 'TB']; let i = 0; while (v >= 1024 && i < u.length - 1) {
    v /= 1024;
    i++;
...
async function copyText(value, notice = '已复制') {
    ...
    try {
        ...
        await showError('复制失败，请长按文本手动复制');
    }
}
...
function showToast(message) {
    ...
    setTimeout(() => { node.classList.remove('show'); setTimeout(() => node.remove(), 180); }, 1500);
}
function appDialog({ title = '提示', message = '', confirmText = '确定', cancelText = '取消', danger = false, input = null, choices = null }) {
    return new Promise(resolve => {
        ...
        root.addEventListener('click', event => { if (event.target === root)
            close(input ? null : false); });
        root.querySelector('[data-dialog-cancel]')?.addEventListener('click', () => close(input ? null : false));
        root.querySelector('[data-dialog-confirm]')?.addEventListener('click', () => { const field = root.querySelector('#app-dialog-input'); if (field && input?.required && !field.value.trim()) {
            field.focus();
...
async function loadOverview(silent = false) {
    ...
    try {
        const d = await api('/api/v1/system/overview');
        state.overview = d;
        if (silent && location.pathname === '/' && document.querySelector('[data-dashboard]'))
            updateDashboard(d);
        else
            render();
    }
    catch (e) {
        state.errors.overview = e.message;
        if (!silent)
            render();
    }
    finally {
        state.loading.overview = false;
        document.querySelector('#refresh-overview svg')?.classList.remove('spin');
    }
}
function dashboardDockerHTML() {
    const data = state.dashboardDocker;
    if (!data)
        return `<div class="dashboard-docker-loading"><div class="spinner"></div><span>正在读取 Docker</span></div>`;
    if (!data.available)
        return `<div class="dashboard-docker-empty"><strong>Docker 不可用</strong><span>${escapeHTML(data.error || '无法连接 Docker Engine')}</span><a data-nav href="/docker">查看详情 ${icon('chevron', 14)}</a></div>`;
    const containers = data.containers || [], running = containers.filter(c => c.state === 'running').length, stopped = containers.length - running;
    return `<div class="dashboard-docker-summary"><div><strong>${running}</strong><span>运行中</span></div><div><strong>${stopped}</strong><span>已停止</span></div><div><strong>${containers.length}</strong><span>总容器</span></div></div><div class="dashboard-docker-list">${containers.slice(0, 4).map(c => `<div><span class="status-dot-small ${c.state === 'running' ? 'success' : 'muted'}"></span><strong>${escapeHTML(containerName(c))}</strong><small>${escapeHTML(c.state === 'running' ? '运行中' : c.status || '已停止')}</small></div>`).join('') || '<span class="muted-text">暂无容器</span>'}</div><div class="card-footer"><span>Docker ${escapeHTML(data.version || '')}</span><a data-nav href="/docker">管理 Docker ${icon('chevron', 14)}</a></div>`;
}
async function loadDashboardDocker(silent = false) {
    if (state.loading.dashboardDocker)
        return;
    state.loading.dashboardDocker = true;
    try {
        const status = await api('/api/v1/docker/status');
        if (!status.available) {
            state.dashboardDocker = { available: false, error: status.error };
            return;
        }
...
function swapDisplay(memory) {
    ...
    if (!total)
        return { value: '未启用', detail: '当前系统没有启用 Swap', percent: 0 };
    if (!used)
        return { value: '未使用', detail: `已启用 · 总量 ${formatBytes(total)}`, percent: 0 };
    return { value: `${percent.toFixed(1)}%`, detail: `${formatBytes(used)} / ${formatBytes(total)}`, percent };
}
function dashboard() {
    const d = state.overview;
    if (!d) {
        if (state.errors.overview)
            return `<div class="page-wrap">${pageHeader('概览', '系统状态读取失败', `<button id="refresh-overview" class="secondary-button compact">${icon('refresh', 17)}重试</button>`)}${errorBox(state.errors.overview)}</div>`;
        queueMicrotask(() => loadOverview());
        return `<div class="page-wrap">${pageHeader('概览', '正在读取服务器状态')}${surfaceLoading('正在采集系统信息')}</div>`;
    }
    if (!state.dashboardDocker && !state.loading.dashboardDocker)
        queueMicrotask(() => loadDashboardDocker());
    const memoryPct = percentOf(d.memory.Used, d.memory.Total), diskPct = percentOf(d.disk.Used, d.disk.Total), swap = swapDisplay(d.memory);
    return `<div class="page-wrap overview-page" data-dashboard>${pageHeader('概览', `${d.hostname} · 服务器运行正常`, `<button id="refresh-overview" class="secondary-button compact">${icon('refresh', 17, state.loading.overview ? 'spin' : '')}<span>刷新</span></button>`)}${errorBox(state.errors.overview)}<section class="live-status-strip"><div><span class="status-dot"></span><strong>实时监控</strong><span data-stream-state>实时推送 · 2 秒</span></div><time data-collected>${formatDate(d.collected_at)}</time></section><section class="overview-kpi-grid">${overviewStatCard('uptime', '运行时间', formatUptime(d.uptime_seconds), '持续在线', 'clock', 'blue')}${overviewStatCard('load', '系统负载', d.load_1.toFixed(2), `5 分钟 ${d.load_5.toFixed(2)} · 15 分钟 ${d.load_15.toFixed(2)}`, 'activity', 'green')}${overviewStatCard('cpu', 'CPU 使用率', `${d.cpu_percent.toFixed(1)}%`, `${d.cpu_cores} 核心`, 'server', 'purple')}${overviewStatCard('memory', '内存使用率', `${memoryPct.toFixed(1)}%`, `${formatBytes(d.memory.Used)} / ${formatBytes(d.memory.Total)}`, 'drive', 'orange')}</section><section class="overview-main-grid"><article class="surface content-panel resource-panel"><div class="panel-heading"><div><h2>资源使用</h2><p>服务器当前资源占用</p></div><a data-nav href="/processes">查看进程 ${icon('chevron', 14)}</a></div>${resourceMeterRow('cpu', 'CPU', `${d.cpu_percent.toFixed(1)}%`, `${d.cpu_cores} 核 · 负载 ${d.load_1.toFixed(2)}`, d.cpu_percent, 'blue')}${resourceMeterRow('memory', '内存', `${memoryPct.toFixed(1)}%`, `${formatBytes(d.memory.Used)} / ${formatBytes(d.memory.Total)}`, memoryPct, 'green')}${resourceMeterRow('disk', '系统盘', `${diskPct.toFixed(1)}%`, `${formatBytes(d.disk.Used)} / ${formatBytes(d.disk.Total)}`, diskPct, 'orange')}${resourceMeterRow('swap', 'Swap', swap.value, swap.detail, swap.percent, 'purple')}</article><article class="surface content-panel system-summary-panel"><div class="panel-heading"><div><h2>系统状态</h2><p>基础运行环境</p></div>${icon('server', 20)}</div><dl class="info-list">${infoListRow('操作系统', d.os)}${infoListRow('内核版本', d.kernel)}${infoListRow('架构', d.architecture)}${infoListRow('实时下载', formatRate(d.network.download_bps))}${infoListRow('实时上传', formatRate(d.network.upload_bps))}</dl></article></section><section class="overview-lower-grid"><article class="surface content-panel dashboard-docker-panel"><div class="panel-heading"><div><h2>Docker 概览</h2><p>每 10 秒同步容器状态</p></div><span class="live-dot"></span></div><div id="dashboard-docker-content">${dashboardDockerHTML()}</div></article><article class="surface content-panel quick-actions-panel"><div class="panel-heading"><div><h2>快捷操作</h2><p>常用管理入口</p></div></div><div class="quick-actions-list"><a data-nav href="/files">${icon('folder', 18)}<span><strong>文件管理</strong><small>浏览、编辑与传输文件</small></span>${icon('chevron', 16)}</a><a data-nav href="/updates">${icon('package', 18)}<span><strong>软件管理</strong><small>检查并安装系统更新</small></span>${icon('chevron', 16)}</a><a data-nav href="/audit">${icon('scroll', 18)}<span><strong>日志审计</strong><small>查看操作记录与系统日志</small></span>${icon('chevron', 16)}</a><a data-nav href="/tools">${icon('terminal', 18)}<span><strong>诊断工具</strong><small>执行固定安全诊断</small></span>${icon('chevron', 16)}</a></div></article></section></div>`;
}
function updateDashboard(d) {
    const memoryPct = percentOf(d.memory.Used, d.memory.Total), diskPct = percentOf(d.disk.Used, d.disk.Total), swap = swapDisplay(d.memory);
    const setKPI = (id, value, detail) => { const root = document.querySelector(`[data-kpi="${id}"]`); if (root) {
        root.querySelector('[data-kpi-value]').textContent = value;
        root.querySelector('[data-kpi-detail]').textContent = detail;
    } };
    const setResource = (id, value, detail, percent) => { const root = document.querySelector(`[data-resource="${id}"]`); if (root) {
        root.querySelector('[data-resource-value]').textContent = value;
        root.querySelector('[data-resource-detail]').textContent = detail;
        root.querySelector('[data-resource-progress]').style.width = `${clamp(percent)}%`;
    } };
    setKPI('uptime', formatUptime(d.uptime_seconds), '持续在线');
    setKPI('load', d.load_1.toFixed(2), `5 分钟 ${d.load_5.toFixed(2)} · 15 分钟 ${d.load_15.toFixed(2)}`);
    setKPI('cpu', `${d.cpu_percent.toFixed(1)}%`, `${d.cpu_cores} 核心`);
    setKPI('memory', `${memoryPct.toFixed(1)}%`, `${formatBytes(d.memory.Used)} / ${formatBytes(d.memory.Total)}`);
    setResource('cpu', `${d.cpu_percent.toFixed(1)}%`, `${d.cpu_cores} 核 · 负载 ${d.load_1.toFixed(2)}`, d.cpu_percent);
    setResource('memory', `${memoryPct.toFixed(1)}%`, `${formatBytes(d.memory.Used)} / ${formatBytes(d.memory.Total)}`, memoryPct);
    setResource('disk', `${diskPct.toFixed(1)}%`, `${formatBytes(d.disk.Used)} / ${formatBytes(d.disk.Total)}`, diskPct);
    setResource('swap', swap.value, swap.detail, swap.percent);
    const collected = document.querySelector('[data-collected]');
    if (collected)
        collected.textContent = formatDate(d.collected_at);
}
function syncOverviewUpdates() {
    stopOverviewUpdates();
    if (!state.authenticated || document.hidden || location.pathname !== '/')
        return;
    ...
    const fallback = () => {
        ...
        const label = document.querySelector('[data-stream-state]');
        if (label)
            label.textContent = `兼容刷新 · ${seconds} 秒`;
    ...
    stream.addEventListener('open', () => { state.overviewStreamFailed = false; const label = document.querySelector('[data-stream-state]'); if (label)
        label.textContent = '实时推送 · 2 秒'; });
    stream.addEventListener('overview', event => {
        try {
            const data = JSON.parse(event.data);
            state.overview = data;
            if (location.pathname === '/' && document.querySelector('[data-dashboard]'))
                updateDashboard(data);
        }
        catch { }
    });
    stream.onerror = () => {
        if (state.overviewStream !== stream)
            return;
        stream.close();
        state.overviewStream = null;
        if (!state.overviewStreamFailed) {
            state.overviewStreamFailed = true;
            fallback();
        }
    };
}
function stopOverviewUpdates() {
    if (state.overviewStream) {
    ...
    if (state.dashboardDockerTimer) {
        clearInterval(state.dashboardDockerTimer);
        state.dashboardDockerTimer = null;
    }
}
function systemPage() {
    const modules = [
...
async function loadServices(query = state.serviceQuery || '') {
    ...
    try {
        state.services = await api(`/api/v1/system/services?query=${encodeURIComponent(query)}`);
    }
    catch (e) {
        state.errors.services = e.message;
    }
    finally {
...
function visibleServices() {
    ...
    if (state.serviceQuery)
        return all;
    if (state.serviceFilter === 'all')
        return all;
    if (state.serviceFilter === 'failed')
        return all.filter(x => x.active === 'failed' || x.sub === 'failed');
    return all.filter(x => x.active === 'active');
}
function servicesPage() {
    const data = state.services;
    if (!data && state.errors.services)
        return `<div class="page-wrap">${pageHeader('服务管理', '无法读取 systemd 服务', `<button id="refresh-services" class="secondary-button compact">重试</button>`)}${errorBox(state.errors.services)}</div>`;
    if (!data) {
        queueMicrotask(() => loadServices());
        return `<div class="page-wrap">${pageHeader('服务管理', '读取 systemd 服务')}${surfaceLoading()}</div>`;
    }
...
async function serviceAction(name, action) { const label = action === 'stop' ? '停止' : action === 'restart' ? '重启' : '启动'; if (['stop', 'restart'].includes(action) && !await askConfirm(`确认${label} ${name}？`, { title: `${label}服务`, confirmText: label, danger: action === 'stop' }))
    return; setBusy(true); try {
    await secureApi('/api/v1/system/services/action', { method: 'POST', body: jsonBody({ name, action }) });
    await loadServices(state.serviceQuery);
    showToast(`服务已${label}`);
}
catch (e) {
...
async function showServiceLogs(name) { state.modal = { title: name, kind: 'logs', content: '正在读取日志…' }; render(); try {
    const data = await api(`/api/v1/system/services/logs?name=${encodeURIComponent(name)}&lines=400`);
    state.modal = { title: `${name} 日志`, kind: 'logs', content: data.logs || '暂无日志' };
    render();
}
catch (e) {
    state.modal = { title: name, kind: 'error', content: e.message };
    render();
} }
async function loadProcesses(silent = false) {
    if (state.loading.processes)
        return;
    state.loading.processes = true;
    state.errors.processes = '';
    try {
        state.processes = await api('/api/v1/system/processes');
        if (!state.processPrimed) {
            state.processPrimed = true;
            setTimeout(() => { if (location.pathname === '/processes')
                loadProcesses(true); }, 1200);
        }
        ;
        if (!silent)
            render();
        ...
        state.errors.processes = e.message;
        if (!silent)
            render();
    }
    finally {
...
function updateProcessRows() { const root = document.querySelector('#process-rows'); if (root)
    root.innerHTML = processRowsHTML(); document.querySelectorAll('[data-process-pid]').forEach(bindProcessButton); }
...
async function signalProcess(pid, signal) { const force = signal === 'kill'; if (!await askConfirm(force ? '强制结束可能导致数据损坏。' : '进程将收到 SIGTERM，可自行清理并退出。', { title: `结束 PID ${pid}`, confirmText: force ? '强制结束' : '正常结束', danger: force }))
    return; try {
    await secureApi('/api/v1/system/processes/action', { method: 'POST', body: jsonBody({ pid, signal }) });
    state.modal = null;
    await loadProcesses();
    showToast('结束信号已发送');
}
...
async function loadNetwork() { state.errors.network = ''; try {
    state.network = await api('/api/v1/system/network');
}
catch (e) {
    state.errors.network = e.message;
}
finally {
    render();
} }
function networkPage() { if (!state.network && !state.errors.network) {
    queueMicrotask(loadNetwork);
    return `<div class="page-wrap">${pageHeader('网络管理', '读取网络接口')}${surfaceLoading()}</div>`;
} return `<div class="page-wrap">${pageHeader('网络管理', '接口地址、累计流量与监听端口', `<button id="refresh-network" class="secondary-button compact">${icon('refresh', 17)}<span>刷新</span></button>`)}${errorBox(state.errors.network)}<section class="interface-grid">${(state.network?.interfaces || []).map(i => `<article class="surface interface-card"><div><strong>${escapeHTML(i.name)}</strong><span>${escapeHTML(i.flags)}</span></div><p>${(i.addresses || []).map(escapeHTML).join('<br>') || '无地址'}</p><small>MTU ${i.mtu} · ↓ ${formatBytes(i.received_bytes)} · ↑ ${formatBytes(i.sent_bytes)}</small></article>`).join('')}</section><section class="result-panel surface"><div class="card-heading">${icon('network', 19)}<strong>监听端口</strong></div><pre>${escapeHTML(state.network?.listening || '未读取到监听端口')}</pre></section></div>`; }
async function loadStorage() { state.errors.storage = ''; try {
    state.storage = await api('/api/v1/system/storage');
}
catch (e) {
    state.errors.storage = e.message;
}
finally {
    render();
} }
function storagePage() {
    if (!state.storage && !state.errors.storage) {
        queueMicrotask(loadStorage);
        return `<div class="page-wrap">${pageHeader('存储管理', '读取文件系统')}${surfaceLoading()}</div>`;
    }
...
async function loadTasks() {
    ...
    state.errors.tasks = '';
    try {
        const [managed, timers] = await Promise.all([api('/api/v1/system/tasks'), api('/api/v1/system/timers')]);
        state.tasks = managed;
        state.timers = timers;
    }
    catch (error) {
        state.errors.tasks = error.message;
    }
    finally {
...
function taskScheduleLabel(task) { if (task.frequency === 'hourly')
    return `每小时 ${String(task.minute).padStart(2, '0')} 分`; if (task.frequency === 'weekly')
    return `每周${'日一二三四五六'[task.weekday]} ${String(task.hour).padStart(2, '0')}:${String(task.minute).padStart(2, '0')}`; return `每天 ${String(task.hour).padStart(2, '0')}:${String(task.minute).padStart(2, '0')}`; }
function tasksPage() {
    if (!state.tasks && !state.errors.tasks) {
        queueMicrotask(loadTasks);
        return `<div class="page-wrap">${pageHeader('计划任务', '读取安全计划任务')}${surfaceLoading()}</div>`;
    }
...
async function taskAction(id, action) {
    if (action === 'delete' && !await askConfirm('删除后对应的 systemd service 和 timer 会一起移除。', { title: '删除计划任务', confirmText: '确认删除', danger: true }))
        return;
    try {
        await secureApi('/api/v1/system/tasks/action', { method: 'POST', body: jsonBody({ id, action }) });
        showToast(action === 'run' ? '任务已开始执行' : '计划任务已更新');
        await loadTasks();
    }
    catch (error) {
...
async function createTask(form) {
    ...
    try {
        await secureApi('/api/v1/system/tasks/create', { method: 'POST', body: jsonBody({ name: f.get('name'), type: f.get('type'), target: f.get('target'), frequency: f.get('frequency'), hour: Number(f.get('hour')), minute: Number(f.get('minute')), weekday: Number(f.get('weekday')) }) });
        state.modal = null;
        showToast('计划任务已创建');
        await loadTasks();
    }
...
async function loadUpdates() {
    ...
    try {
        state.aptPreflight = await api('/api/v1/system/apt/preflight');
        state.updates = state.aptPreflight;
    }
    catch (e) {
        state.errors.updates = e.message;
    }
    finally {
...
function updatesPage() {
    ...
    if (!x && !state.errors.updates) {
        queueMicrotask(loadUpdates);
        return `<div class="page-wrap">${pageHeader('软件管理', '执行安全预检')}${surfaceLoading('模拟 APT 升级')}</div>`;
    }
...
async function aptSearch(query) {
    state.aptQuery = query;
    state.loading.aptSearch = true;
    try {
        const out = await api(`/api/v1/system/apt/search?q=${encodeURIComponent(query)}`);
        state.aptSearchResults = out.packages || out || [];
    }
    catch (error) {
        await showError(error.message);
    }
    finally {
        state.loading.aptSearch = false;
        render();
    }
}
async function aptAction(action, packages = []) {
    ...
    if (!await askConfirm(detail, { title: labels[action], confirmText: '启动后台任务', danger }))
        return;
    try {
        const job = await startBackgroundJob(`apt.${action}`, { packages });
        state.modal = { title: labels[action], kind: 'job-progress', job };
        render();
...
async function loadBackgroundJobs(renderAfter = true) {
    ...
    try {
        ...
        state.backgroundJobs = out.jobs || [];
    }
    catch (error) {
        state.errors.jobs = error.message;
    }
    finally {
...
async function monitorBackgroundJob(id, title) {
    for (let count = 0; count < 1800; count++) {
        ...
        if (state.modal?.kind === 'job-progress' && state.modal?.job?.id === id) {
            state.modal = { ...state.modal, job };
            render();
        }
        if (job.status === 'success' || job.status === 'failed') {
            const output = jobResultOutput(job);
            if (state.modal?.kind === 'job-progress' && state.modal?.job?.id === id) {
                state.modal = { title: job.status === 'success' ? `${title}完成` : `${title}失败`, kind: 'logs', content: output || job.error || '任务结束' };
                render();
            }
            else
                showToast(job.status === 'success' ? `${title}已完成` : `${title}失败`);
...
function backgroundJobPanel(prefix) {
    if (!state.backgroundJobs && !state.jobsLoading)
        queueMicrotask(() => loadBackgroundJobs());
...
async function loadHostSettings() { state.errors.host = ''; try {
    state.hostSettings = await api('/api/v1/system/host');
}
catch (error) {
    state.errors.host = error.message;
}
finally {
    render();
} }
function hostPage() {
    ...
    if (!x && !state.errors.host) {
        queueMicrotask(loadHostSettings);
        return `<div class="page-wrap">${pageHeader('主机设置', '正在读取系统配置')}${surfaceLoading()}</div>`;
    }
    ...
    return `<div class="page-wrap host-settings-page">${pageHeader('主机设置', '所有修改都经过校验，并在关键操作前创建快照', `<button id="refresh-host" class="secondary-button compact">${icon('refresh', 17)}刷新</button>`)}${errorBox(state.errors.host)}<section class="host-settings-grid"><form id="host-basic-form" class="surface host-settings-card"><div class="panel-heading"><div><h2>基础信息</h2><p>主机名与系统时区</p></div>${icon('server', 20)}</div><label>主机名<input name="hostname" value="${escapeHTML(x?.hostname || '')}" required></label><label>时区<input name="timezone" value="${escapeHTML(x?.timezone || 'UTC')}" placeholder="Asia/Shanghai" required></label><button class="primary-button" type="submit">保存基础设置</button></form><form id="host-dns-form" class="surface host-settings-card"><div class="panel-heading"><div><h2>系统 DNS</h2><p>${x?.systemd_resolved ? '由 systemd-resolved 管理' : '未检测到 systemd-resolved'}</p></div>${icon('network', 20)}</div><label>DNS 服务器<textarea name="dns" rows="4" placeholder="每行一个，例如 1.1.1.1">${escapeHTML((x?.dns || []).join('\n'))}</textarea></label><button class="primary-button" type="submit" ${x?.systemd_resolved ? '' : 'disabled'}>测试并保存</button></form><article class="surface host-settings-card"><div class="panel-heading"><div><h2>Swap</h2><p>${x?.swap?.managed ? '由 LukePanel 管理' : '系统交换空间'}</p></div><span class="state-pill ${x?.swap?.enabled ? 'success' : 'muted'}">${x?.swap?.enabled ? '已启用' : '未启用'}</span></div><div class="setting-value"><strong>${formatBytes(x?.swap?.used || 0)}</strong><span>/ ${formatBytes(x?.swap?.total || 0)}</span></div>${x?.swap?.managed ? `<button id="host-swap-delete" class="danger-button">删除 LukePanel 管理的 Swap</button>` : `<form id="host-swap-form" class="compact-inline-form"><label>大小（MB）<input name="size_mb" type="number" min="256" max="32768" value="2048"></label><button class="primary-button" type="submit">创建 Swap</button></form>`}</article><article class="surface host-settings-card ntp-status-card"><div class="panel-heading"><div><h2>时间同步</h2><p>系统时钟与 TOTP 的基础</p></div><span class="state-pill ${ntpTone}">${ntpState}</span></div><dl class="info-list compact">${infoListRow('服务', n.service || '未检测到时间同步服务')}${infoListRow('服务状态', n.service_active ? '运行中' : n.service_unit ? '未运行' : '由系统环境提供')}${infoListRow('时区', n.timezone || x?.timezone || '-')}${infoListRow('时间服务器', n.server_name || n.server_address || '由服务自动选择')}${infoListRow('上次同步', n.last_sync || '暂无记录')}</dl><button id="toggle-ntp" data-enabled="${!!n.enabled}" class="${n.enabled ? 'secondary-button' : 'primary-button'}" ${n.available ? '' : 'disabled'}>${n.enabled ? '关闭时间同步' : '开启时间同步'}</button></article></section><section class="surface kernel-profile-card"><div class="panel-heading"><div><h2>内核优化</h2><p>只应用固定、可审计的 sysctl 模板</p></div><span class="current-profile">当前：${escapeHTML(sys.label || '系统默认')}</span></div><div class="kernel-meta-grid"><span>拥塞控制 <strong>${escapeHTML(sys.congestion_control || '未读取')}</strong></span><span>队列算法 <strong>${escapeHTML(sys.default_qdisc || '未读取')}</strong></span><span>Swappiness <strong>${Number(sys.swappiness || 0)}</strong></span><span>BBR <strong>${sys.bbr ? '已启用' : '未启用'}</strong></span></div><div class="profile-option-grid">${sysctlPresetOption('balanced', '均衡', '适合大多数通用服务器', sys.preset)}${sysctlPresetOption('network', '网络吞吐', '启用 BBR 与 fq，偏向网络服务', sys.preset)}${sysctlPresetOption('low-memory', '小内存 VPS', '降低脏页比例，减少内存压力', sys.preset)}${sysctlPresetOption('reset', '恢复系统默认', '删除 LukePanel 管理的 sysctl 文件', sys.preset, true)}</div>${sys.managed ? `<small class="managed-note">配置文件：${escapeHTML(sys.config_path || '/etc/sysctl.d/99-lukepanel.conf')}</small>` : '<small class="managed-note">当前没有 LukePanel 管理的内核优化配置。</small>'}</section></div>`;
}
async function hostMutation(endpoint, body, method = 'POST') {
    setBusy(true);
    try {
        await secureApi(`/api/v1/system/host/${endpoint}`, { method, body: body === null ? undefined : jsonBody(body) });
        showToast('主机设置已更新');
...
async function loadSnapshots() { state.errors.snapshots = ''; try {
    const [out, scheduled] = await Promise.all([api('/api/v1/system/snapshots'), api('/api/v1/backup/scheduled')]);
    state.snapshots = out.snapshots || out || [];
    state.scheduledBackups = scheduled;
}
catch (error) {
    state.errors.snapshots = error.message;
}
finally {
    render();
} }
function snapshotsPage() {
    if (!state.snapshots && !state.errors.snapshots) {
        queueMicrotask(loadSnapshots);
        return `<div class="page-wrap">${pageHeader('配置快照', '读取快照')}${surfaceLoading()}</div>`;
    }
...
async function snapshotAction(id, action) {
    if (!await askConfirm(action === 'restore' ? '恢复会覆盖当前配置，但恢复前还会再创建一个回滚点。' : '确认永久删除这个快照？', { title: action === 'restore' ? '恢复配置快照' : '删除配置快照', confirmText: action === 'restore' ? '确认恢复' : '确认删除', danger: true }))
        return;
    try {
        await secureApi('/api/v1/system/snapshots', { method: 'POST', body: jsonBody({ id, action }) });
        showToast(action === 'restore' ? '快照已恢复' : '快照已删除');
        await loadSnapshots();
    }
    catch (error) {
        await showError(error.message);
...
async function exportPanelBackup() {
    ...
    if (response.status === 403) {
        await requestElevation();
        response = await fetch('/api/v1/backup/export', { credentials: 'same-origin' });
    }
    if (!response.ok) {
...
async function importPanelBackup(file) {
    ...
    if (!await askConfirm('恢复会替换账号、安全设置和面板数据，但保留当前监听地址与 Agent 通信参数。建议先导出当前备份。', { title: '恢复 LukePanel 完整备份', confirmText: '校验并恢复', danger: true }))
        return;
...
async function loadDocker() {
    ...
    try {
        ...
        if (!status.available) {
            state.docker = null;
            state.dockerImages = null;
            state.dockerNetworks = null;
            state.dockerVolumes = null;
            state.dockerCompose = null;
            return;
        }
        ...
        state.dockerNetworks = networks;
        state.dockerVolumes = volumes;
        state.dockerCompose = compose;
    }
    catch (e) {
        state.errors.docker = e.message;
    }
    finally {
        state.loading.docker = false;
        render();
    }
}
...
function syncDockerStats() {
    if (state.dockerStatsTimer) {
        clearInterval(state.dockerStatsTimer);
        state.dockerStatsTimer = null;
    }
    if (!state.authenticated || document.hidden || location.pathname !== '/docker' || state.dockerTab !== 'containers')
        return;
    loadDockerStats();
...
function dockerStatHTML(id) {
    ...
    if (!stat)
        return '<span class="container-stat-muted">正在读取资源…</span>';
...
async function dockerAction(id, action) {
    ...
    if (['stop', 'restart', 'kill', 'remove'].includes(action) && !await askConfirm(`确认${labels[action]}这个容器？`, { title: `${labels[action]}容器`, confirmText: labels[action], danger: ['kill', 'remove'].includes(action) }))
        return;
    setBusy(true);
    try {
        await secureApi('/api/v1/docker/action', { method: 'POST', body: jsonBody({ id, action }) });
        await loadDocker();
        showToast(`容器已${labels[action]}`);
    }
    catch (e) {
        await showError(e.message);
    }
...
function collectDockerEnv(form) { return [...form.querySelectorAll('.docker-env-row')].map(row => { const key = row.querySelector('[data-env-key]').value.trim(), value = row.querySelector('[data-env-value]').value; if (!key && !value)
    return null; if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key))
    throw new Error(`环境变量名 ${key || '(空)'} 无效`); return `${key}=${value}`; }).filter(Boolean); }
function collectDockerPorts(form) { return [...form.querySelectorAll('.docker-port-row')].map((row, index) => { const host_ip = row.querySelector('[data-port-host-ip]').value.trim(), host_port = row.querySelector('[data-port-host-port]').value.trim(), container_port = row.querySelector('[data-port-container-port]').value.trim(), protocol = row.querySelector('[data-port-protocol]').value; if (!host_port && !container_port && !host_ip)
    return null; if (!/^\d{1,5}$/.test(container_port))
    throw new Error(`第 ${index + 1} 个端口缺少有效的容器端口`); if (host_port && !/^\d{1,5}$/.test(host_port))
    throw new Error(`第 ${index + 1} 个主机端口无效`); return { host_ip, host_port, container_port, protocol }; }).filter(Boolean); }
function collectDockerMounts(form) { return [...form.querySelectorAll('.docker-mount-row')].map((row, index) => { const type = row.querySelector('[data-mount-type]').value, source = row.querySelector('[data-mount-source]').value.trim(), target = row.querySelector('[data-mount-target]').value.trim(); if (!source && !target)
    return null; if (!source || !target)
    throw new Error(`第 ${index + 1} 个挂载需要同时填写来源和容器路径`); return { type, source, target, read_only: row.querySelector('[data-mount-readonly]').checked }; }).filter(Boolean); }
async function installDocker() {
    if (!await askConfirm('将使用 Debian / Ubuntu 官方软件源安装 Docker，并启用 docker.service。安装期间请不要关闭页面。', { title: '快捷安装 Docker', confirmText: '开始安装' }))
        return;
    setBusy(true);
    try {
        const result = await secureApi('/api/v1/docker/install', { method: 'POST', body: '{}' });
        showToast('Docker 安装完成');
        if (result.output)
            state.modal = { title: 'Docker 安装结果', kind: 'logs', content: String(result.output) };
        await loadDocker();
    }
    catch (error) {
        await showError(error.message);
...
function addDockerVisualRow(type) { const targets = { env: ['#docker-env-rows', dockerEnvRow], port: ['#docker-port-rows', dockerPortRow], mount: ['#docker-mount-rows', dockerMountRow] }, item = targets[type]; if (!item)
    return; const root = document.querySelector(item[0]); if (!root)
    return; root.insertAdjacentHTML('beforeend', item[1]()); root.lastElementChild?.querySelector('[data-remove-docker-row]')?.addEventListener('click', event => event.currentTarget.closest('.visual-row')?.remove()); }
async function openDockerEditor(id, title) { state.modal = { title: `编辑 ${title}`, kind: 'loading', content: '' }; render(); try {
    const spec = await api(`/api/v1/docker/inspect?id=${encodeURIComponent(id)}`);
    state.dockerEdit = spec;
    state.modal = { title: `编辑 ${spec.name}`, kind: 'docker-edit', spec };
    render();
}
catch (error) {
    state.modal = { title: '无法读取容器配置', kind: 'error', content: error.message };
    render();
} }
async function submitDockerEdit() {
    const form = document.querySelector('#docker-edit-form');
    if (!form)
        return;
    const f = new FormData(form), button = form.querySelector('button[type=submit]');
    let request;
    try {
        request = { id: state.dockerEdit.id, name: String(f.get('name') || '').trim(), image: String(f.get('image') || '').trim(), env: collectDockerEnv(form), cmd: lines(f.get('cmd')), entrypoint: lines(f.get('entrypoint')), working_dir: String(f.get('working_dir') || '').trim(), user: String(f.get('user') || '').trim(), hostname: String(f.get('hostname') || '').trim(), restart_policy: String(f.get('restart_policy') || 'no'), restart_maximum_retry_count: Number(f.get('restart_maximum_retry_count') || 0), network_mode: String(f.get('network_mode') || 'default'), privileged: f.get('privileged') === 'on', ports: collectDockerPorts(form), mounts: collectDockerMounts(form), start: f.get('start') === 'on' };
    }
    catch (error) {
        await showError(error.message);
        return;
    }
    if (!await askConfirm(`LukePanel 会先保留旧容器，只有新容器成功启动后才清理备份。`, { title: `重建 ${request.name}`, confirmText: '安全重建' }))
        return;
    button.disabled = true;
    button.textContent = '正在安全重建…';
    try {
        const result = await secureApi('/api/v1/docker/recreate', { method: 'POST', body: jsonBody(request) });
        state.modal = null;
        state.dockerEdit = null;
        await loadDocker();
        showToast(result.warning || '容器配置已更新');
    }
    catch (error) {
        await showError(error.message);
    }
        ...
        button.disabled = false;
        button.textContent = '保存并重建';
    }
}
async function composeAction(project, action) {
    const labels = { up: '启动或更新', restart: '重启', stop: '停止', down: '停止并移除', pull: '拉取镜像' };
    if (['down', 'restart'].includes(action) && !await askConfirm(`确认${labels[action]} Compose 项目 ${project}？`, { title: 'Compose 操作', confirmText: labels[action], danger: action === 'down' }))
        return;
    setBusy(true);
    try {
        const result = await secureApi('/api/v1/docker/compose/action', { method: 'POST', body: jsonBody({ project, action }) });
        if (result.output)
            showToast(result.output.slice(0, 120));
        await loadDocker();
    }
    catch (e) {
        await showError(e.message);
...
async function pullImage() {
    const reference = await askText('输入完整镜像名称', { title: '拉取 Docker 镜像', value: 'nginx:latest', placeholder: '例如 nginx:latest', confirmText: '开始拉取' });
    if (!reference)
        return;
    setBusy(true);
    try {
        await secureApi('/api/v1/docker/images/pull', { method: 'POST', body: jsonBody({ reference }) });
        state.dockerTab = 'images';
        await loadDocker();
        showToast('镜像拉取完成');
    }
    catch (e) {
...
async function deleteDockerResource(kind, value, label = value) {
    if (!await askConfirm(`正在使用的资源会被 Docker 拒绝删除。`, { title: `删除 ${label}`, confirmText: '确认删除', danger: true }))
        return;
    const map = { image: ['/api/v1/docker/images/delete', { id: value }], network: ['/api/v1/docker/networks/delete', { id: value }], volume: ['/api/v1/docker/volumes/delete', { name: value }] };
    setBusy(true);
    try {
        await secureApi(map[kind][0], { method: 'POST', body: jsonBody(map[kind][1]) });
        await loadDocker();
        showToast('Docker 资源已删除');
    }
    catch (e) {
        await showError(e.message);
    }
...
async function openDockerCleanup() {
    state.modal = { title: 'Docker 清理', kind: 'loading' };
    render();
    try {
        state.dockerCleanupPreview = await api('/api/v1/docker/cleanup/preview');
        state.modal = { title: 'Docker 清理', kind: 'docker-cleanup', preview: state.dockerCleanupPreview };
        render();
    }
    catch (error) {
        state.modal = { title: '无法分析 Docker 空间', kind: 'error', content: error.message };
        render();
    }
}
async function submitDockerCleanup(form) {
    ...
    if (!await askConfirm(mode === 'deep' ? '深度清理会删除更多未使用镜像，确认继续？' : '只清理明确未使用的 Docker 资源。', { title: '执行 Docker 清理', confirmText: '开始清理', danger: mode === 'deep' || includeVolumes }))
        return;
    try {
        const result = await secureApi('/api/v1/docker/cleanup', { method: 'POST', body: jsonBody({ mode, include_volumes: includeVolumes }) });
        state.modal = null;
        await loadDocker();
        showToast(`清理完成，释放 ${formatBytes(result.space_reclaimed || 0)}`);
    }
    catch (error) {
        await showError(error.message);
    }
}
async function createDockerNetwork(form) { const f = new FormData(form), button = form.querySelector('button'); button.disabled = true; try {
    await secureApi('/api/v1/docker/networks/create', { method: 'POST', body: jsonBody({ name: f.get('name'), driver: f.get('driver'), subnet: f.get('subnet'), gateway: f.get('gateway'), internal: f.get('internal') === 'on' }) });
    state.modal = null;
    state.dockerTab = 'networks';
    await loadDocker();
    showToast('Docker 网络已创建');
}
catch (error) {
    await showError(error.message);
}
...
async function createDockerVolume(form) { const f = new FormData(form), button = form.querySelector('button'); button.disabled = true; try {
    await secureApi('/api/v1/docker/volumes/create', { method: 'POST', body: jsonBody({ name: f.get('name'), driver: f.get('driver') }) });
    state.modal = null;
    state.dockerTab = 'volumes';
    await loadDocker();
    showToast('Docker 存储卷已创建');
}
catch (error) {
    await showError(error.message);
}
...
async function loadFiles(path = '/') {
    ...
    state.errors.files = '';
    try {
        state.files = await api(`/api/v1/files?path=${encodeURIComponent(path)}`);
        state.fileContent = null;
    }
    catch (e) {
        state.errors.files = e.message;
    }
    finally {
...
async function loadRecycle() {
    state.loading.recycle = true;
    state.errors.recycle = '';
    try {
        state.recycle = await api('/api/v1/files/recycle');
    }
    catch (e) {
        state.errors.recycle = e.message;
    }
    finally {
        state.loading.recycle = false;
        render();
    }
}
...
function fileBreadcrumb(path) {
    if (!path || path === '/')
        return '<span class="breadcrumb-current">根目录</span>';
...
function filesPage() {
    ...
    if (state.fileView === 'recycle') {
        if (!state.recycle && !state.errors.recycle) {
            queueMicrotask(loadRecycle);
            return `<div class="page-wrap">${pageHeader('文件管理', '正在读取回收站')}${surfaceLoading()}</div>`;
        }
        const entries = state.recycle?.entries || [];
        return `<div class="page-wrap files-product-page">${pageHeader('文件管理', '已删除内容可以恢复或永久清理')}<div class="tab-bar surface file-tabs"><button data-file-view="files">文件</button><button class="active" data-file-view="recycle">回收站 <span>${entries.length}</span></button></div>${errorBox(state.errors.recycle)}<section class="compact-resource-list surface">${entries.map(item => `<article class="compact-list-row"><div class="compact-list-icon">${icon(item.is_dir ? 'folder' : 'file', 20)}</div><div class="compact-list-main"><strong>${escapeHTML(item.name)}</strong><span title="${escapeHTML(item.original_path)}">${escapeHTML(item.original_path)}</span><small>${formatDate(item.deleted_at)}${item.is_dir ? ' · 文件夹' : ` · ${formatBytes(item.size)}`}</small></div><div class="row-actions"><button class="secondary-button compact" data-recycle-action="restore" data-recycle-id="${escapeHTML(item.id)}">恢复</button><button class="danger-button compact" data-recycle-action="purge" data-recycle-id="${escapeHTML(item.id)}">永久删除</button></div></article>`).join('') || '<div class="empty-list">回收站是空的</div>'}</section></div>`;
    }
    if (!l && !state.errors.files) {
        queueMicrotask(() => loadFiles('/'));
        return `<div class="page-wrap">${pageHeader('文件管理', '正在读取文件系统')}${surfaceLoading()}</div>`;
    }
...
async function createEntry() {
    if (!state.files)
        return;
    const choice = await chooseAction('新建内容', [{ label: '新建文件夹', description: '创建一个空目录' }, { label: '新建文件', description: '创建一个空文本文件' }]);
    if (choice === false || choice === null)
        return;
    const type = choice === 0 ? 'folder' : 'file', name = await askText('', { title: type === 'folder' ? '文件夹名称' : '文件名称', placeholder: type === 'folder' ? '例如 config' : '例如 config.yaml' });
    if (!name)
        return;
    const base = state.files.path.replace(/\/$/, ''), path = `${base}/${name}`;
    try {
        await secureApi(type === 'folder' ? '/api/v1/files/mkdir' : '/api/v1/files/create', { method: 'POST', body: jsonBody({ path }) });
        await loadFiles(state.files.path);
        showToast(`${type === 'folder' ? '文件夹' : '文件'}已创建`);
    }
    catch (e) {
        await showError(e.message);
    }
...
async function uploadSelected(files, preservePaths = false) {
    ...
    try {
        for (const file of list) {
            ...
            form.append('directory', state.files.path);
            form.append('relative_path', preservePaths ? (file.webkitRelativePath || file.name) : file.name);
        ...
        await loadFiles(state.files.path);
        showToast(`已上传 ${completed} 个${preservePaths ? '文件夹内文件' : '文件'}`);
    }
    catch (e) {
        await showError(`已上传 ${completed}/${list.length} 个文件。${e.message}`);
    }
    finally {
        setBusy(false);
    }
...
async function uploadAndExtractZIP(files) {
    ...
    if (!file || !state.files)
        return;
    if (!await askConfirm(`将 ${file.name} 解压到 ${state.files.path}，同名文件默认不覆盖。`, { title: '上传并解压 ZIP', confirmText: '开始解压' }))
        return;
    const form = new FormData();
    form.append('directory', state.files.path);
    form.append('overwrite', 'false');
    ...
    setBusy(true);
    try {
        const result = await secureApi('/api/v1/files/archive/extract', { method: 'POST', body: form });
        await loadFiles(state.files.path);
        showToast(`已解压 ${result.files || 0} 个文件`);
    }
    catch (error) {
        await showError(error.message);
    }
...
async function fileMutation(action, item) {
    ...
    try {
        if (action === 'rename') {
            const next = await askText('', { title: '重命名', value: name, placeholder: '新名称' });
            if (!next || next === name)
                return;
            ...
            const destination = await askText('', { title: action === 'copy' ? '复制到完整路径' : '移动到完整路径', value: suggestion, placeholder: '/opt/example' });
            if (!destination || destination === source)
                return;
            await secureApi(`/api/v1/files/${action}`, { method: 'POST', body: jsonBody({ source, destination }) });
        }
        else if (action === 'chmod') {
            const current = String(item.mode || '').match(/[0-7]{3,4}$/)?.[0] || '644', mode = await askText('', { title: '修改八进制权限', value: current, placeholder: '文件常用 644，目录常用 755' });
            if (!mode)
                return;
            await secureApi('/api/v1/files/chmod', { method: 'POST', body: jsonBody({ path: source, mode }) });
        }
        else if (action === 'chown') {
            const value = await askText('格式：用户:用户组，可只填写其中一项，例如 root:root 或 :www-data', { title: '修改文件所有者', placeholder: 'root:root' });
            if (!value)
                return;
            const [owner = '', group = ''] = String(value).split(':', 2);
            await secureApi('/api/v1/files/chown', { method: 'POST', body: jsonBody({ path: source, owner: owner.trim(), group: group.trim() }) });
        }
        else if (action === 'delete') {
            if (!await askConfirm(source, { title: '移入回收站', confirmText: '确认删除', danger: true }))
                return;
...
async function recycleAction(id, action) {
    if (action === 'purge' && !await askConfirm('永久删除后无法恢复。', { title: '永久删除', confirmText: '永久删除', danger: true }))
        return;
    try {
        await secureApi('/api/v1/files/recycle', { method: 'POST', body: jsonBody({ id, action, destination: '' }) });
        await loadRecycle();
        showToast(action === 'restore' ? '文件已恢复' : '已永久删除');
    }
    catch (error) {
        if (action === 'restore' && String(error.message).includes('恢复目标已存在')) {
            const item = (state.recycle?.entries || []).find(entry => entry.id === id), destination = await askText('', { title: '原位置已有同名文件', value: item?.original_path ? `${item.original_path}-恢复` : '', placeholder: '输入新的完整恢复路径' });
            if (!destination)
                return;
            try {
                await secureApi('/api/v1/files/recycle', { method: 'POST', body: jsonBody({ id, action, destination }) });
                ...
                await showError(nextError.message);
            }
            ;
            return;
        }
...
async function loadFileBackups(path) {
    state.modal = { title: '历史版本', kind: 'loading' };
    render();
    try {
        state.fileBackups = await api(`/api/v1/files/backups?path=${encodeURIComponent(path)}`);
        state.modal = { title: '历史版本', kind: 'file-backups', path, backups: state.fileBackups.versions || [] };
        render();
    }
    catch (error) {
        state.modal = { title: '无法读取历史版本', kind: 'error', content: error.message };
        render();
    }
}
async function showFileBackupDiff(path, id) { state.modal = { title: '对比历史版本', kind: 'loading' }; render(); try {
    state.fileDiff = await api(`/api/v1/files/backups/diff?path=${encodeURIComponent(path)}&id=${encodeURIComponent(id)}`);
    state.modal = { title: '版本差异', kind: 'file-diff', path, id, diff: state.fileDiff };
    render();
}
catch (error) {
    state.modal = { title: '无法对比版本', kind: 'error', content: error.message };
    render();
} }
async function restoreFileBackup(path, id) { if (!await askConfirm('恢复前会自动备份当前内容，可以再次回滚。', { title: '恢复历史版本', confirmText: '恢复此版本' }))
    return; try {
    await secureApi('/api/v1/files/backups/restore', { method: 'POST', body: jsonBody({ path, id }) });
    showToast('历史版本已恢复');
    await openFile(path);
}
catch (error) {
...
async function runTool(form) { const f = new FormData(form), button = form.querySelector('button'), result = document.querySelector('#tool-result'), pre = result.querySelector('pre'); button.disabled = true; button.textContent = '测试中…'; result.hidden = false; pre.textContent = '正在执行…'; try {
    const data = await api('/api/v1/tools/run', { method: 'POST', body: jsonBody({ tool: form.dataset.tool, target: f.get('target'), port: Number(f.get('port') || 0) }) });
    pre.textContent = (data.output || '完成') + `\n\n耗时：${data.duration_ms} ms`;
}
catch (e) {
...
async function runDiagnostic() { const button = document.querySelector('#run-diagnostic'), result = document.querySelector('#tool-result'), pre = result.querySelector('pre'); button.disabled = true; button.textContent = '诊断中…'; result.hidden = false; pre.textContent = '正在收集固定诊断项…'; try {
    const data = await api('/api/v1/tools/run', { method: 'POST', body: jsonBody({ tool: 'diagnostic', target: '', port: 0 }) });
    pre.textContent = (data.output || '完成') + `\n\n耗时：${data.duration_ms} ms`;
...
async function loadAudit() { state.loading.audit = true; state.errors.audit = ''; try {
    const q = state.auditQuery, params = new URLSearchParams();
    Object.entries(q).forEach(([key, value]) => { if (value !== '' && value !== null && value !== undefined)
        params.set(key, String(value)); });
    const logParams = new URLSearchParams({ lines: '600' });
    if (state.logSource)
        logParams.set('unit', state.logSource);
    const [audit, logs] = await Promise.all([api('/api/v1/audit?' + params.toString()), api('/api/v1/logs/system?' + logParams.toString())]);
    state.audit = audit;
    state.systemLogs = logs;
}
catch (e) {
    state.errors.audit = e.message;
}
finally {
...
function auditPage() {
    if (!state.audit && !state.errors.audit) {
        queueMicrotask(loadAudit);
        return `<div class="page-wrap">${pageHeader('日志审计', '正在读取操作记录')}${surfaceLoading()}</div>`;
    }
...
async function loadSSH(user = '') {
    state.loading.ssh = true;
    state.errors.ssh = '';
    try {
        const [status, users] = await Promise.all([api('/api/v1/ssh/status'), api('/api/v1/ssh/users')]);
        state.ssh = status;
        state.sshUsers = users;
        if (!user)
            user = state.sshUser || users.users?.[0]?.name || '';
        state.sshUser = user;
        state.sshKeys = user ? await api(`/api/v1/ssh/keys?user=${encodeURIComponent(user)}`) : { keys: [] };
    }
    catch (e) {
        state.errors.ssh = e.message;
    }
    finally {
        state.loading.ssh = false;
        render();
    }
}
function sshPage() {
    if (!state.ssh && !state.errors.ssh) {
        queueMicrotask(() => loadSSH());
        return `<div class="page-wrap">${pageHeader('SSH 管理', '正在读取 SSH 状态')}${surfaceLoading()}</div>`;
    }
    ...
    const pending = status.pending_new_port ? `<div class="alert warning ssh-port-pending">${icon('alert', 18)}<div><strong>SSH 端口等待确认</strong><p>新端口 ${escapeHTML(status.pending_new_port)} 与旧端口 ${escapeHTML(status.pending_old_port)} 同时监听。请先用新端口登录，再确认关闭旧端口。</p><div class="inline-actions"><button id="ssh-port-keep-new" class="primary-button compact">新端口已验证</button><button id="ssh-port-revert" class="danger-button compact">恢复旧端口</button></div></div></div>` : '';
    const serviceLabel = status.available ? (status.running === false ? '已停止' : '运行中') : '不可用';
    return `<div class="page-wrap ssh-page ssh-product-page">${pageHeader('SSH 管理', '安全管理端口、登录方式、公钥和 Linux 用户', `<button id="refresh-ssh" class="secondary-button compact">${icon('refresh', 17)}<span>刷新</span></button>`)}${errorBox(state.errors.ssh)}${pending}<section class="ssh-overview-grid"><article class="surface content-panel ssh-status-panel"><div class="panel-heading"><div><h2>OpenSSH</h2><p>当前服务与登录策略</p></div><span class="state-pill ${status.available && status.running !== false ? 'success' : 'warning'}">${escapeHTML(serviceLabel)}</span></div><div class="ssh-metrics-grid"><div><span>端口</span><strong>${escapeHTML(status.port || '-')}</strong></div><div><span>Root 登录</span><strong>${escapeHTML(status.permit_root_login || '-')}</strong></div><div><span>密码登录</span><strong>${passwordEnabled ? '已启用' : '已关闭'}</strong></div><div><span>授权公钥</span><strong>${totalKeys} 把</strong></div></div><div class="ssh-password-action">${passwordEnabled ? `<button id="ssh-disable-password" class="danger-button" ${totalKeys < 1 ? 'disabled' : ''}>${icon('shield', 17)}关闭密码登录</button>` : `<button id="ssh-enable-password" class="secondary-button">恢复密码登录</button>`}<small>${passwordEnabled ? (totalKeys ? '关闭前请先在另一个终端测试公钥登录。' : '没有检测到公钥，暂不允许关闭密码登录。') : '当前仅允许密钥等非密码方式登录。'}</small></div></article><form id="ssh-settings-form" class="surface content-panel ssh-settings-panel"><div class="panel-heading"><div><h2>连接设置</h2><p>修改端口时采用双端口过渡</p></div>${icon('wrench', 19)}</div><div class="two-column-form-grid"><label>SSH 端口<input name="port" type="number" min="1" max="65535" value="${escapeHTML(String(status.port || '22').split(' ')[0])}" required></label><label>Root 登录<select name="permit_root_login"><option value="prohibit-password" ${status.permit_root_login === 'prohibit-password' ? 'selected' : ''}>只允许密钥（推荐）</option><option value="no" ${status.permit_root_login === 'no' ? 'selected' : ''}>完全禁止</option><option value="yes" ${status.permit_root_login === 'yes' ? 'selected' : ''}>允许</option><option value="forced-commands-only" ${status.permit_root_login === 'forced-commands-only' ? 'selected' : ''}>仅强制命令</option></select></label></div><div class="setting-toggle-list"><label class="toggle-field"><input name="allow_tcp_forwarding" type="checkbox" ${boolValue(status.allow_tcp_forwarding) ? 'checked' : ''}><span><strong>TCP 转发</strong><small>代理、隧道与 Remote 开发可能需要</small></span></label><label class="toggle-field"><input name="allow_agent_forwarding" type="checkbox" ${boolValue(status.allow_agent_forwarding) ? 'checked' : ''}><span><strong>Agent 转发</strong><small>不使用跳板机时建议关闭</small></span></label><label class="toggle-field"><input name="x11_forwarding" type="checkbox" ${boolValue(status.x11_forwarding) ? 'checked' : ''}><span><strong>X11 转发</strong><small>无图形程序时建议关闭</small></span></label></div><button class="primary-button" type="submit">校验并应用</button></form></section><section class="surface content-panel ssh-keys-panel"><div class="panel-heading"><div><h2>授权公钥</h2><p>管理所选用户的 ~/.ssh/authorized_keys</p></div><div class="key-toolbar"><select id="ssh-user">${users.map(user => `<option value="${escapeHTML(user.name)}" ${user.name === state.sshUser ? 'selected' : ''}>${escapeHTML(user.name)} · ${user.key_count} 把</option>`).join('')}</select><button id="ssh-generate-key" class="primary-button compact">${icon('key', 17)}生成密钥</button></div></div><div class="key-list">${keys.map(key => `<article class="key-list-row"><div class="key-type-badge">${escapeHTML(key.type.replace('ssh-', ''))}</div><div><strong>${escapeHTML(key.comment || '未命名公钥')}</strong><p>${escapeHTML(key.fingerprint)}</p><small>${escapeHTML(key.preview)}</small></div><button class="danger-button compact" data-ssh-key-delete="${escapeHTML(key.id)}">删除</button></article>`).join('') || '<div class="empty-list">这个用户还没有公钥</div>'}</div><details class="add-key-panel"><summary>${icon('plus', 16)}添加已有公钥</summary><form id="ssh-key-form" class="ssh-key-form"><label>OpenSSH 公钥<textarea name="key" rows="4" placeholder="ssh-ed25519 AAAA... iPhone" required></textarea></label><button class="secondary-button" type="submit">验证并添加</button></form></details></section><section class="surface content-panel linux-users-panel linux-users-panel-product"><div class="panel-heading"><div><h2>Linux 用户</h2><p>仅显示可登录用户；新用户默认锁定密码</p></div><button id="create-linux-user" class="primary-button compact">${icon('plus', 16)}新建用户</button></div><div class="linux-user-list">${users.map(user => `<article><div class="user-avatar">${escapeHTML(user.name.slice(0, 1).toUpperCase())}</div><div><strong>${escapeHTML(user.name)}${user.uid === 0 ? ' · root' : ''}</strong><p>UID ${user.uid} · ${escapeHTML(user.home)} · ${escapeHTML(user.shell)}</p><small>${user.key_count || 0} 把公钥 · ${user.sudo ? '拥有 sudo 权限' : '普通用户'}</small></div>${user.name !== 'root' ? `<div class="row-actions"><button class="secondary-button compact" data-user-sudo="${escapeHTML(user.name)}" data-enabled="${!!user.sudo}">${user.sudo ? '移除 sudo' : '授予 sudo'}</button><button class="danger-button compact" data-user-delete="${escapeHTML(user.name)}">删除</button></div>` : '<span class="state-pill">系统管理员</span>'}</article>`).join('') || '<div class="empty-list">没有可管理的登录用户</div>'}</div></section></div>`;
}
async function addSSHKey(form) { const button = form.querySelector('button'), key = new FormData(form).get('key'); button.disabled = true; button.textContent = '添加中…'; try {
    await secureApi('/api/v1/ssh/keys/add', { method: 'POST', body: jsonBody({ user: state.sshUser, key }) });
    form.reset();
    await loadSSH(state.sshUser);
    showToast('公钥已添加');
}
catch (e) {
    await showError(e.message);
    ...
    button.disabled = false;
    button.textContent = '添加已有公钥';
} }
async function deleteSSHKey(id) { if (!await askConfirm('请确保还有其他可用登录方式，避免把自己锁在服务器外。', { title: '删除 SSH 公钥', confirmText: '确认删除', danger: true }))
    return; try {
    await secureApi('/api/v1/ssh/keys/delete', { method: 'POST', body: jsonBody({ user: state.sshUser, id }) });
    await loadSSH(state.sshUser);
    showToast('SSH 公钥已删除');
}
catch (e) {
...
async function toggleSSHPassword(enabled) {
    ...
    if (!await askConfirm(message, { title: enabled ? '恢复密码登录' : '关闭密码登录', confirmText: enabled ? '确认恢复' : '我已测试密钥，关闭密码', danger: !enabled }))
        return;
    try {
        await secureApi('/api/v1/ssh/password', { method: 'POST', body: jsonBody({ enabled }) });
        showToast(enabled ? '密码登录已恢复' : '密码登录已关闭');
        await loadSSH(state.sshUser);
    }
    catch (error) {
        await showError(error.message);
...
async function generateSSHKey(form) {
    ...
    if (passphrase !== confirmValue) {
        ...
        error.textContent = '两次输入的私钥口令不一致';
        error.hidden = false;
        return;
    }
    button.disabled = true;
    ...
    try {
        const result = await secureApi('/api/v1/ssh/keys/generate', { method: 'POST', body: jsonBody({ user: state.sshUser, comment: f.get('comment'), passphrase }) });
        state.modal = { title: '立即保存私钥', kind: 'ssh-generated-key', key: result };
        await loadSSH(state.sshUser);
        state.modal = { title: '立即保存私钥', kind: 'ssh-generated-key', key: result };
        render();
    }
...
async function loadGitHubAuth(silent = false) {
    try {
        state.githubAuth = await api('/api/v1/github/auth/status');
    }
    catch (error) {
        state.githubAuth = { connected: false, error: error.message };
    }
    if (!silent)
        render();
}
async function loadGitHub(owner, repo) {
    ...
    owner = String(owner || defaults.owner || '').trim();
    repo = String(repo || defaults.repo || '').trim();
    if (!owner || !repo) {
        state.github = null;
        state.errors.github = '';
        render();
    ...
    try {
        state.github = await api(`/api/v1/github/summary?owner=${encodeURIComponent(owner)}&repo=${encodeURIComponent(repo)}`);
    }
    catch (e) {
        state.errors.github = e.message;
    }
    finally {
...
function workflowStatus(run) { if (run.status !== 'completed')
    return '运行中'; return run.conclusion === 'success' ? '成功' : run.conclusion || '未知'; }
function githubAuthCard() {
    const auth = state.githubAuth;
    if (!auth)
        return `<section class="surface github-auth-card"><div class="spinner"></div><span>检查 GitHub 登录状态…</span></section>`;
    if (auth.connected)
...
function githubImportHTML(data) {
    if (!state.githubAuth?.connected)
        return `<section class="surface github-import-panel muted-panel"><h2>ZIP 推送</h2><p>连接 GitHub 后，可以像 Working Copy 一样上传源码 ZIP，预览变更并 Commit + Push。</p></section>`;
...
function githubPage() {
    if (!githubHelperEnabled())
        return `<div class="page-wrap github-page">${pageHeader('GitHub 助手', '可选功能，不使用时不会发起任何 GitHub 请求')}<section class="surface optional-feature-empty"><div class="feature-illustration">${icon('github', 34)}</div><h2>GitHub 助手入口已隐藏</h2><p>这是内置功能，无需安装。显示入口后可使用设备登录、仓库管理、Actions 和 Release。</p><button id="github-helper-install" class="primary-button">显示 GitHub 助手入口</button><a data-nav href="/tools" class="secondary-button">返回常用工具</a></section></div>`;
    const defaults = githubDefaults(), data = state.github;
    if (!state.githubAuth && !state.loading.githubAuth) {
        state.loading.githubAuth = true;
        queueMicrotask(async () => { await loadGitHubAuth(true); state.loading.githubAuth = false; render(); });
    }
    const latest = data?.latest_release, tagSuggestion = latest?.tag_name ? nextVersionSuggestion(latest.tag_name) : 'v2.0.0';
...
async function startGitHubDeviceFlow(form) {
    ...
    try {
        ...
        if (button.isConnected) {
            button.disabled = false;
            button.innerHTML = `${icon('github', 18)}使用 GitHub 设备登录`;
        }
    }
}
function scheduleGitHubPoll() { if (state.githubFlowTimer)
    clearTimeout(state.githubFlowTimer); if (!state.githubFlow?.flow_id)
    return; const delay = Math.max(2, Number(state.githubFlow.interval || 5)) * 1000; state.githubFlowTimer = setTimeout(pollGitHubDeviceFlow, delay); }
async function pollGitHubDeviceFlow() { if (!state.githubFlow?.flow_id)
    return; try {
    const result = await api('/api/v1/github/auth/device/poll', { method: 'POST', body: jsonBody({ flow_id: state.githubFlow.flow_id }) });
    if (result.status === 'authorized') {
        state.githubFlow = null;
        state.githubAuth = { connected: true, ...result };
        showToast(`已连接 GitHub @${result.login}`);
        const defaults = githubDefaults();
        if (defaults.owner && defaults.repo)
            await loadGitHub(defaults.owner, defaults.repo);
        else
            render();
        return;
    }
    if (['expired', 'denied'].includes(result.status)) {
        state.githubFlow = null;
        await showError(result.message || 'GitHub 登录已取消或过期');
        render();
        return;
    }
    state.githubFlow.interval = result.retry_after || state.githubFlow.interval;
    scheduleGitHubPoll();
}
catch (error) {
    state.githubFlow = null;
...
async function disconnectGitHub() { if (!await askConfirm('Token 会立即从内存移除。', { title: '断开 GitHub 登录', confirmText: '断开连接' }))
    return; await api('/api/v1/github/auth/disconnect', { method: 'POST' }); state.githubAuth = { connected: false }; state.githubImportPlan = null; render(); }
async function previewGitHubImport(form) { const f = new FormData(form), button = form.querySelector('button'); localStorage.setItem('github-owner', String(f.get('owner'))); localStorage.setItem('github-repo', String(f.get('repo'))); localStorage.setItem('github-branch', String(f.get('branch'))); button.disabled = true; button.textContent = '上传并比较中…'; try {
    state.githubImportPlan = await api('/api/v1/github/import/preview', { method: 'POST', body: f });
    render();
    showToast('差异预览已生成');
}
catch (error) {
    ...
    button.disabled = false;
    button.textContent = '上传并预览差异';
} }
async function commitGitHubImport(form) { const f = new FormData(form), button = form.querySelector('button'), plan = state.githubImportPlan; if (!plan)
    return; if (!await askConfirm(`将 ${plan.added} 个新增、${plan.modified} 个修改文件推送到 ${plan.owner}/${plan.repo}:${plan.branch}。`, { title: 'Commit 并 Push', confirmText: '确认推送' }))
    return; button.disabled = true; button.textContent = '正在 Commit + Push…'; try {
    const result = await secureApi('/api/v1/github/import/commit', { method: 'POST', body: jsonBody({ plan_id: plan.id, message: f.get('message') }) });
    state.githubImportPlan = null;
    showToast(`已推送 ${result.files} 个文件`);
    window.open(result.html_url, '_blank', 'noopener');
    await loadGitHub(plan.owner, plan.repo);
}
catch (error) {
    await showError(error.message);
}
    ...
    button.disabled = false;
    button.textContent = `Commit 并 Push 到 ${plan.branch}`;
} }
async function createGitHubTag(form) { const f = new FormData(form), button = form.querySelector('button'), tag = String(f.get('tag') || ''); if (!await askConfirm(`创建 ${tag} 后 Release Actions 会自动开始。`, { title: '创建版本标签', confirmText: '创建 Tag' }))
    return; button.disabled = true; button.textContent = '正在创建…'; try {
    await secureApi('/api/v1/github/tag', { method: 'POST', body: jsonBody({ owner: state.github.owner, repo: state.github.name, tag, targetSHA: f.get('sha') }) });
    showToast('标签已创建，Release Actions 将自动运行');
    setTimeout(() => loadGitHub(state.github.owner, state.github.name), 2500);
}
catch (e) {
    await showError(e.message);
}
    ...
    button.disabled = false;
    button.textContent = '创建 Tag 并触发发布';
} }
async function createGitHubBranch(form) { const f = new FormData(form), button = form.querySelector('button'), name = String(f.get('name') || '').trim(); button.disabled = true; try {
    await secureApi('/api/v1/github/branch', { method: 'POST', body: jsonBody({ owner: state.github.owner, repo: state.github.name, name, source: f.get('source') }) });
    showToast(`分支 ${name} 已创建`);
    localStorage.setItem('github-branch', name);
...
async function createGitHubPullRequest(form) { const f = new FormData(form), button = form.querySelector('button'); if (!await askConfirm(`${f.get('head')} → ${f.get('base')}`, { title: '创建 Pull Request', confirmText: '创建 PR' }))
    return; button.disabled = true; try {
    const result = await secureApi('/api/v1/github/pull', { method: 'POST', body: jsonBody({ owner: state.github.owner, repo: state.github.name, title: f.get('title'), body: f.get('body'), head: f.get('head'), base: f.get('base') }) });
    showToast(`PR #${result.number} 已创建`);
    window.open(result.html_url, '_blank', 'noopener');
}
catch (error) {
    await showError(error.message);
...
async function rerunGitHubAction(runID) { if (!await askConfirm('只会重新运行失败的任务，不会重跑已成功任务。', { title: '重试 GitHub Actions', confirmText: '重新运行' }))
    return; try {
    await secureApi('/api/v1/github/rerun', { method: 'POST', body: jsonBody({ owner: state.github.owner, repo: state.github.name, run_id: Number(runID) }) });
    showToast('已请求重试失败任务');
...
async function loadSecurity() { if (state.loading.security)
    return; state.loading.security = true; state.errors.security = ''; try {
    const [settings, sessions, totp, report, passkeys, devices, allowlist, notifications, firewall, fail2ban] = await Promise.all([api('/api/v1/settings'), api('/api/v1/auth/sessions'), api('/api/v1/auth/totp/status'), api('/api/v1/security/status'), api('/api/v1/auth/passkeys'), api('/api/v1/auth/trusted-devices'), api('/api/v1/security/ip-allowlist'), api('/api/v1/security/login-notifications'), api('/api/v1/security/firewall'), api('/api/v1/security/fail2ban')]);
    Object.assign(state, { settings, sessions, totpStatus: totp, securityReport: report, passkeys, trustedDevices: devices, ipAllowlist: allowlist, loginNotifications: notifications, firewall, fail2ban });
    state.username = settings.admin_user || state.username;
}
catch (e) {
    state.errors.security = e.message;
}
finally {
...
async function refreshFirewallStatus() {
    const status = await api('/api/v1/security/firewall');
...
function securityPage() {
    if ((!state.settings || !state.sessions || !state.totpStatus || !state.securityReport) && !state.errors.security)
        queueMicrotask(loadSecurity);
...
function updatePasswordStrength() {
    const input = document.querySelector('#new-password'), confirm = document.querySelector('#confirm-password'), form = document.querySelector('#password-form');
    if (!input || !form)
        return;
    ...
    strength.querySelector('span').style.width = `${assessment.score}%`;
    strength.querySelector('strong').textContent = input.value ? `${assessment.label} · ${assessment.score}%` : '尚未输入';
...
async function changeUsername(form) { const f = new FormData(form), button = form.querySelector('button[type=submit]'), error = form.querySelector('.form-error'); button.disabled = true; error.hidden = true; try {
    const result = await api('/api/v1/auth/account', { method: 'PATCH', body: jsonBody({ username: String(f.get('username') || '').trim(), current_password: f.get('current_password') }) });
    state.username = result.username;
    state.settings.admin_user = result.username;
    localStorage.setItem('lukepanel-login-user', result.username);
    form.querySelector('[name=current_password]').value = '';
    showToast('用户名已更新，其他设备已退出');
}
catch (e) {
...
async function installFail2Ban() { if (!await askConfirm('将安装 Fail2ban，启用 SSH 防暴力破解，并自动把当前访问 IP 与内网网段加入忽略列表。', { title: '启用 SSH 防暴力破解', confirmText: '安装并启用' }))
    return; setBusy(true); try {
    await secureApi('/api/v1/security/fail2ban/install', { method: 'POST', body: '{}' });
    showToast('Fail2ban 已启用');
    await loadSecurity();
}
catch (error) {
    await showError(error.message);
...
async function enableAutomaticUpdates() { if (!await askConfirm('将安装 unattended-upgrades，并自动安装 Debian/Ubuntu 安全更新。不会自动重启服务器。', { title: '启用自动安全更新', confirmText: '安装并启用' }))
    return; setBusy(true); try {
    await secureApi('/api/v1/security/auto-updates/enable', { method: 'POST', body: '{}' });
    showToast('自动安全更新已启用');
    await loadSecurity();
}
catch (error) {
...
async function startTOTPSetup() { try {
    state.totpSetup = await secureApi('/api/v1/auth/totp/setup', { method: 'POST' });
    state.modal = { title: '开启两步验证', kind: 'totp-setup', setup: state.totpSetup };
    render();
}
catch (error) {
    await showError(error.message);
} }
async function confirmTOTPSetup(form) { const code = String(new FormData(form).get('code') || ''), button = form.querySelector('button'); button.disabled = true; try {
    await secureApi('/api/v1/auth/totp/confirm', { method: 'POST', body: jsonBody({ code }) });
    const codes = state.totpSetup?.recovery_codes || [];
    state.totpSetup = null;
    state.modal = { title: '保存恢复码', kind: 'recovery-codes', codes };
    await loadSecurity();
    state.modal = { title: '保存恢复码', kind: 'recovery-codes', codes };
    render();
}
...
async function disableTOTP() { const code = await askText('输入当前 6 位验证码或一个恢复码', { title: '关闭两步验证', placeholder: '验证码或恢复码', confirmText: '关闭', required: true }); if (!code)
    return; if (!await askConfirm('关闭后登录只依赖密码。', { title: '确认关闭两步验证', confirmText: '确认关闭', danger: true }))
    return; try {
    await secureApi('/api/v1/auth/totp/disable', { method: 'POST', body: jsonBody({ code }) });
    showToast('两步验证已关闭');
    await loadSecurity();
}
catch (error) {
...
async function regenerateRecoveryCodes() { const code = await askText('输入当前 6 位验证码或一个恢复码', { title: '重新生成恢复码', placeholder: '验证码或恢复码' }); if (!code)
    return; try {
    const result = await secureApi('/api/v1/auth/totp/recovery', { method: 'POST', body: jsonBody({ code }) });
    state.modal = { title: '新的恢复码', kind: 'recovery-codes', codes: result.recovery_codes || [] };
    render();
    await loadSecurity();
    state.modal = { title: '新的恢复码', kind: 'recovery-codes', codes: result.recovery_codes || [] };
    render();
...
async function openComposeConfig(project) { state.modal = { title: `${project} · Compose`, kind: 'loading' }; render(); try {
    const config = await api(`/api/v1/docker/compose/config?project=${encodeURIComponent(project)}`);
    state.composeConfig = config;
    state.modal = { title: `${project} · Compose 配置`, kind: 'compose-config', config };
    render();
}
catch (error) {
    state.modal = { title: '读取 Compose 失败', kind: 'error', content: error.message };
    render();
} }
async function saveComposeConfig(form) { const files = {}; form.querySelectorAll('[data-compose-path]').forEach(area => files[area.dataset.composePath] = area.value); const button = form.querySelector('button[type=submit]'); button.disabled = true; button.textContent = '正在验证…'; try {
    const out = await secureApi('/api/v1/docker/compose/config', { method: 'PUT', body: jsonBody({ project: state.composeConfig.project, files, deploy: new FormData(form).get('deploy') === 'on' }) });
    state.modal = { title: 'Compose 保存结果', kind: 'logs', content: out.output || '配置已保存并通过校验' };
    await loadDocker();
    state.modal = { title: 'Compose 保存结果', kind: 'logs', content: out.output || '配置已保存并通过校验' };
    render();
}
...
async function buildDockerImage(form) {
    ...
    try {
        ...
        state.modal = { title: `构建镜像 ${f.get('tag')}`, kind: 'job-progress', job };
        render();
...
async function confirmSSHPort(keepNew) { if (!await askConfirm(keepNew ? '确认你已经从另一个终端通过新端口成功登录？确认后旧端口将关闭。' : '将删除新端口并恢复旧端口。', { title: keepNew ? '确认新 SSH 端口' : '恢复旧 SSH 端口', confirmText: '确认', danger: !keepNew }))
    return; try {
    await secureApi('/api/v1/ssh/port/confirm', { method: 'POST', body: jsonBody({ keep_new: keepNew }) });
    showToast(keepNew ? '已关闭旧端口' : '已恢复旧端口');
    await loadSSH(state.sshUser);
}
catch (error) {
    await showError(error.message);
} }
async function createGitHubRelease(form) { const f = new FormData(form), defaults = githubDefaults(), data = state.github; if (!data)
    return; try {
    const out = await secureApi('/api/v1/github/release', { method: 'POST', body: jsonBody({ owner: data.owner || defaults.owner, repo: data.name || defaults.repo, tag: f.get('tag'), name: f.get('name'), body: f.get('body'), draft: f.get('draft') === 'on', prerelease: f.get('prerelease') === 'on' }) });
    showToast(`Release ${out.tag_name || f.get('tag')} 已创建`);
    await loadGitHub(data.owner, data.name);
}
catch (error) {
    await showError(error.message);
} }
async function mergeGitHubPullRequest(button) {
    ...
    if (!await askConfirm('GitHub 分支保护和 Actions 检查仍然生效；检查不通过时不会强行合并。', { title: `合并 PR #${button.dataset.githubPrMerge}`, confirmText: '确认合并', danger: false }))
        return;
    try {
...
function modalHTML() {
    ...
    let body = '', footer = '';
    if (m.kind === 'loading')
        body = '<div class="modal-loading"><div class="spinner"></div></div>';
    else if (m.kind === 'logs') {
        body = `<pre class="modal-log">${escapeHTML(m.content)}</pre>`;
        footer = `<footer><button class="secondary-button compact" id="copy-modal-log">${icon('copy', 16)}复制全部</button><button class="primary-button compact" id="modal-done">完成</button></footer>`;
    }
    else if (m.kind === 'error')
        body = `<div class="alert error modal-error">${icon('alert', 18)}${escapeHTML(m.content)}</div>`;
    ...
    else if (m.kind === 'docker-network')
        body = `<form id="docker-network-form" class="dialog-form"><label>网络名称<input name="name" placeholder="例如 app-network" required></label><label>驱动<select name="driver"><option value="bridge">bridge（推荐）</option><option value="macvlan">macvlan</option><option value="ipvlan">ipvlan</option></select></label><label>子网（可选）<input name="subnet" placeholder="172.30.0.0/16"></label><label>网关（可选）<input name="gateway" placeholder="172.30.0.1"></label><label class="checkbox-row"><input type="checkbox" name="internal"><span>仅内部网络，不访问外网</span></label><button class="primary-button" type="submit">创建网络</button></form>`;
    else if (m.kind === 'docker-volume')
        body = `<form id="docker-volume-form" class="dialog-form"><label>存储卷名称<input name="name" placeholder="例如 app-data" required></label><label>驱动<input name="driver" value="local" required></label><button class="primary-button" type="submit">创建存储卷</button></form>`;
    else if (m.kind === 'task-create')
        body = `<form id="task-create-form" class="dialog-form"><label>任务名称<input name="name" placeholder="例如每天重启 mosdns" required></label><label>安全任务类型<select name="type" id="task-type"><option value="service-restart">重启 systemd 服务</option><option value="docker-restart">重启 Docker 容器</option><option value="docker-cleanup-safe">安全清理 Docker</option><option value="panel-backup">完整面板备份</option></select></label><label id="task-target-label">目标名称<input name="target" placeholder="例如 mosdns.service" required></label><label>执行频率<select name="frequency" id="task-frequency"><option value="daily">每天</option><option value="weekly">每周</option><option value="hourly">每小时</option></select></label><div class="time-fields"><label>小时<input name="hour" type="number" min="0" max="23" value="4"></label><label>分钟<input name="minute" type="number" min="0" max="59" value="0"></label><label id="task-weekday-label">星期<select name="weekday"><option value="1">周一</option><option value="2">周二</option><option value="3">周三</option><option value="4">周四</option><option value="5">周五</option><option value="6">周六</option><option value="0">周日</option></select></label></div><div class="release-warning">${icon('shield', 17)}不支持自定义 Shell，避免计划任务变成远程 WebShell。</div><button class="primary-button" type="submit">创建计划任务</button></form>`;
    else if (m.kind === 'file-backups')
        body = `<div class="backup-list">${(m.backups || []).map(version => `<article><div><strong>${formatDate(version.created_at)}</strong><small>${formatBytes(version.size)} · ${escapeHTML(version.id)}</small></div><div><button class="secondary-button compact" data-backup-diff="${escapeHTML(version.id)}">对比</button><button class="primary-button compact" data-backup-restore="${escapeHTML(version.id)}">恢复</button></div></article>`).join('') || '<div class="empty-list">还没有历史版本。文件每次在线保存前会自动备份。</div>'}</div>`;
    else if (m.kind === 'file-diff') {
        body = `<div class="diff-note">备份时间：${formatDate(m.diff?.backup_time)}${m.diff?.truncated ? ' · 内容较长，已截断' : ''}</div><pre class="file-diff">${escapeHTML(m.diff?.diff || '没有差异')}</pre>`;
        footer = `<footer><button class="secondary-button" id="back-to-backups">返回版本列表</button><button class="primary-button" data-backup-restore="${escapeHTML(m.id)}">恢复此版本</button></footer>`;
    }
    else if (m.kind === 'totp-setup')
    ...
    else if (m.kind === 'recovery-codes') {
        const text = (m.codes || []).join('\n');
        body = `<div class="recovery-codes"><div class="alert warning">${icon('alert', 18)}每个恢复码只能用一次。请保存到密码管理器，不要只留在这台服务器上。</div><pre>${escapeHTML(text)}</pre><button class="secondary-button" data-copy-text="${escapeHTML(text)}">复制全部恢复码</button><button class="secondary-button" id="download-recovery-codes">下载文本文件</button></div>`;
        footer = `<footer><button class="primary-button" id="modal-done">我已安全保存</button></footer>`;
    }
    else if (m.kind === 'ssh-key-generate')
        body = `<form id="ssh-key-generate-form" class="dialog-form"><p>为 <strong>${escapeHTML(m.user || '')}</strong> 生成 ED25519 密钥，并自动加入授权列表。</p><label>密钥备注<input name="comment" value="${escapeHTML(`${m.user || 'user'}@LukePanel-${new Date().toISOString().slice(0, 10)}`)}" maxlength="128" required></label><label>私钥口令（推荐）<input name="passphrase" type="password" autocomplete="new-password" ${passwordInputAttributes()} placeholder="留空则私钥不加密"></label><label>再次输入私钥口令<input name="confirm" type="password" autocomplete="new-password" ${passwordInputAttributes()}></label><div class="form-error" hidden></div><div class="release-warning">${icon('shield', 17)}私钥只返回一次。设置口令更安全，但连接时需要输入口令或使用密钥代理。</div><button class="primary-button" type="submit">生成并加入授权列表</button></form>`;
    else if (m.kind === 'ssh-generated-key') {
        const key = m.key || {};
        body = `<div class="generated-key-panel"><div class="alert warning">${icon('alert', 18)}这是唯一一次显示私钥。关闭前请立即下载，并确认文件已保存。</div><dl class="info-list"><div><dt>文件名</dt><dd>${escapeHTML(key.filename || 'id_ed25519')}</dd></div><div><dt>指纹</dt><dd><code>${escapeHTML(key.fingerprint || '-')}</code></dd></div><div><dt>备注</dt><dd>${escapeHTML(key.comment || '-')}</dd></div></dl><div class="generated-key-actions"><button id="download-generated-private-key" class="primary-button">${icon('download', 17)}下载私钥</button><button id="copy-generated-public-key" class="secondary-button">${icon('copy', 17)}复制公钥</button></div><details><summary>查看公钥</summary><pre>${escapeHTML(key.public_key || '')}</pre></details></div>`;
        footer = `<footer><button class="primary-button" id="modal-done">我已保存私钥</button></footer>`;
    }
    else if (m.kind === 'docker-edit') {
        const x = m.spec;
        if (x.compose_managed) {
            body = `<div class="compose-managed-note">${icon('alert', 20)}<h3>这个容器由 Docker Compose 管理</h3><p>直接重建会让配置和 Compose 文件失去同步。请编辑下面的 Compose YAML，再回到 Docker 页面点“启动/更新”。</p>${(x.compose_files || []).map(path => `<button class="secondary-button" data-open-compose-file="${escapeHTML(path)}">${icon('file', 17)}${escapeHTML(path)}</button>`).join('') || '<span>未读取到 Compose 文件路径</span>'}</div>`;
        }
        else {
...
function bindShell() {
    ...
    let searchTimer;
    document.querySelector('#service-search')?.addEventListener('input', e => { state.serviceQuery = e.target.value; clearTimeout(searchTimer); searchTimer = setTimeout(() => loadServices(state.serviceQuery), 350); });
    ...
    const aptSearchForm = document.querySelector('#apt-search-form');
    if (aptSearchForm)
        aptSearchForm.onsubmit = event => { event.preventDefault(); aptSearch(String(new FormData(aptSearchForm).get('q') || '')); };
    ...
    const hostBasicForm = document.querySelector('#host-basic-form');
    if (hostBasicForm)
        hostBasicForm.onsubmit = async (event) => { event.preventDefault(); const f = new FormData(hostBasicForm); await hostMutation('hostname', { hostname: f.get('hostname') }); await hostMutation('timezone', { timezone: f.get('timezone') }); };
    const hostDNSForm = document.querySelector('#host-dns-form');
    if (hostDNSForm)
        hostDNSForm.onsubmit = event => { event.preventDefault(); const servers = String(new FormData(hostDNSForm).get('dns') || '').split(/\s+/).filter(Boolean); hostMutation('dns', { servers }); };
    const hostSwapForm = document.querySelector('#host-swap-form');
    if (hostSwapForm)
        hostSwapForm.onsubmit = event => { event.preventDefault(); hostMutation('swap', { size_mb: Number(new FormData(hostSwapForm).get('size_mb')) }); };
    document.querySelector('#host-swap-delete')?.addEventListener('click', async () => { if (await askConfirm('确认删除 LukePanel 管理的 Swap 文件？', { title: '删除 Swap', confirmText: '确认删除', danger: true }))
        hostMutation('swap', null, 'DELETE'); });
    ...
    document.querySelectorAll('[data-file-view]').forEach(button => button.onclick = () => { state.fileView = button.dataset.fileView; if (state.fileView === 'recycle' && !state.recycle)
        loadRecycle();
    else
        render(); });
    ...
    document.querySelector('#file-search')?.addEventListener('input', event => {
        state.fileFilter = event.target.value.toLowerCase();
        let shown = 0;
        document.querySelectorAll('.file-row').forEach(row => { const match = row.textContent.toLowerCase().includes(state.fileFilter); row.hidden = !match; if (match)
            shown++; });
        const count = document.querySelector('.file-search span');
        if (count)
            count.textContent = `${shown} / ${state.files?.entries?.length || 0}`;
    });
    ...
    document.querySelector('#github-helper-install')?.addEventListener('click', () => { if (localStorage.getItem('github-helper-v07') !== '1') {
        localStorage.removeItem('github-owner');
        localStorage.removeItem('github-repo');
        localStorage.removeItem('github-branch');
        localStorage.setItem('github-helper-v07', '1');
    } localStorage.setItem('github-helper-enabled', '1'); showToast('GitHub 助手入口已显示'); render(); });
    document.querySelector('#github-helper-remove')?.addEventListener('click', async () => { if (!await askConfirm('隐藏入口不会删除授权，也不会修改任何 GitHub 仓库。', { title: '隐藏 GitHub 助手入口', confirmText: '隐藏入口' }))
        return; localStorage.removeItem('github-helper-enabled'); state.github = null; state.githubAuth = null; if (location.pathname === '/github')
        navigate('/tools');
    else
        render(); });
    ...
    document.querySelector('#audit-search')?.addEventListener('input', event => {
        state.auditFilter = event.target.value.toLowerCase();
        let shown = 0;
        document.querySelectorAll('.audit-row').forEach(row => { const match = row.textContent.toLowerCase().includes(state.auditFilter); row.hidden = !match; if (match)
            shown++; });
        const count = document.querySelector('.audit-search span');
        if (count)
            count.textContent = `${shown} / ${state.audit?.events?.length || 0}`;
    });
    ...
    const sshSettingsForm = document.querySelector('#ssh-settings-form');
    if (sshSettingsForm)
        sshSettingsForm.onsubmit = async (event) => { event.preventDefault(); const f = new FormData(sshSettingsForm); try {
            const result = await secureApi('/api/v1/ssh/settings', { method: 'POST', body: jsonBody({ port: Number(f.get('port')), permit_root_login: f.get('permit_root_login'), allow_tcp_forwarding: f.get('allow_tcp_forwarding') === 'on', allow_agent_forwarding: f.get('allow_agent_forwarding') === 'on', x11_forwarding: f.get('x11_forwarding') === 'on' }) });
            showToast(result.message || 'SSH 设置已应用');
            await loadSSH(state.sshUser);
        }
        catch (error) {
            await showError(error.message);
    ...
    const sshForm = document.querySelector('#ssh-key-form');
    if (sshForm)
        sshForm.onsubmit = event => { event.preventDefault(); addSSHKey(sshForm); };
    ...
    const githubRepoForm = document.querySelector('#github-repo-form');
    if (githubRepoForm)
        githubRepoForm.onsubmit = event => { event.preventDefault(); const f = new FormData(githubRepoForm); loadGitHub(String(f.get('owner')).trim(), String(f.get('repo')).trim()); };
    const githubConnectForm = document.querySelector('#github-connect-form');
    if (githubConnectForm)
        githubConnectForm.onsubmit = event => { event.preventDefault(); startGitHubDeviceFlow(githubConnectForm); };
    ...
    const githubImportForm = document.querySelector('#github-import-form');
    if (githubImportForm)
        githubImportForm.onsubmit = event => { event.preventDefault(); previewGitHubImport(githubImportForm); };
    const githubImportCommitForm = document.querySelector('#github-import-commit-form');
    if (githubImportCommitForm)
        githubImportCommitForm.onsubmit = event => { event.preventDefault(); commitGitHubImport(githubImportCommitForm); };
    const githubTagForm = document.querySelector('#github-tag-form');
    if (githubTagForm)
        githubTagForm.onsubmit = event => { event.preventDefault(); createGitHubTag(githubTagForm); };
    const githubBranchForm = document.querySelector('#github-branch-form');
    if (githubBranchForm)
        githubBranchForm.onsubmit = event => { event.preventDefault(); createGitHubBranch(githubBranchForm); };
    const githubPRForm = document.querySelector('#github-pr-form');
    if (githubPRForm)
        githubPRForm.onsubmit = event => { event.preventDefault(); createGitHubPullRequest(githubPRForm); };
    const githubReleaseForm = document.querySelector('#github-release-form');
    if (githubReleaseForm)
        githubReleaseForm.onsubmit = event => { event.preventDefault(); createGitHubRelease(githubReleaseForm); };
    ...
    if (usernameForm)
        usernameForm.onsubmit = event => { event.preventDefault(); changeUsername(usernameForm); };
    ...
    if (interval) {
        interval.value = String(state.settings?.auto_refresh_seconds || 5);
        interval.onchange = async () => { try {
            await api('/api/v1/settings', { method: 'PATCH', body: jsonBody({ auto_refresh_seconds: Number(interval.value) }) });
            state.settings.auto_refresh_seconds = Number(interval.value);
            syncOverviewUpdates();
            showToast('刷新策略已保存');
        }
        catch (e) {
    ...
    document.querySelector('#revoke-sessions')?.addEventListener('click', async () => { if (!await askConfirm('当前设备会继续登录，其他设备会立即失效。', { title: '退出其他设备', confirmText: '确认退出' }))
        return; try {
        const r = await api('/api/v1/auth/sessions', { method: 'DELETE' });
        showToast(`已退出 ${r.revoked} 个会话`);
        await loadSecurity();
    }
    catch (e) {
        await showError(e.message);
    ...
    if (passwordForm) {
        ...
        passwordForm.onsubmit = async (e) => { e.preventDefault(); const f = new FormData(e.currentTarget), message = document.querySelector('#password-message'), button = e.currentTarget.querySelector('button[type=submit]'), next = String(f.get('next') || ''), confirmValue = String(f.get('confirm') || ''), assessment = passwordAssessment(next, state.settings?.admin_user || state.username); message.hidden = true; if (next !== confirmValue) {
            message.textContent = '两次输入的新密码不一致';
            message.hidden = false;
            return;
        } if (!assessment.ok) {
            message.textContent = '新密码仍不符合强度要求';
            message.hidden = false;
            return;
        } button.disabled = true; button.textContent = '正在保存…'; try {
            await api('/api/v1/auth/password', { method: 'POST', body: jsonBody({ current_password: f.get('current'), new_password: next }) });
            message.className = 'form-success';
            message.textContent = '密码已更新，其他设备已退出';
            message.hidden = false;
    ...
    const composeConfigForm = document.querySelector('#compose-config-form');
    if (composeConfigForm)
        composeConfigForm.onsubmit = event => { event.preventDefault(); saveComposeConfig(composeConfigForm); };
    const imageBuildForm = document.querySelector('#image-build-form');
    if (imageBuildForm)
        imageBuildForm.onsubmit = event => { event.preventDefault(); buildDockerImage(imageBuildForm); };
    document.querySelectorAll('[data-search-result]').forEach(button => button.onclick = () => { const path = button.dataset.searchResult, isDir = button.dataset.directory === 'true'; state.modal = null; if (isDir)
        loadFiles(path);
    else
        openFile(path); });
    document.querySelector('#docker-edit-form')?.addEventListener('submit', event => { event.preventDefault(); submitDockerEdit(); });
    const sshKeyGenerateForm = document.querySelector('#ssh-key-generate-form');
    if (sshKeyGenerateForm)
        sshKeyGenerateForm.onsubmit = event => { event.preventDefault(); generateSSHKey(sshKeyGenerateForm); };
    ...
    if (taskForm) {
        taskForm.onsubmit = event => { event.preventDefault(); createTask(taskForm); };
        const type = taskForm.querySelector('#task-type'), frequency = taskForm.querySelector('#task-frequency'), syncTaskForm = () => { const target = taskForm.querySelector('#task-target-label'), weekday = taskForm.querySelector('#task-weekday-label'), hour = taskForm.querySelector('[name=hour]'); target.hidden = ['docker-cleanup-safe', 'panel-backup'].includes(type.value); target.querySelector('input').required = !target.hidden; weekday.hidden = frequency.value !== 'weekly'; hour.closest('label').hidden = frequency.value === 'hourly'; };
        type.onchange = syncTaskForm;
        frequency.onchange = syncTaskForm;
        syncTaskForm();
    }
    const totpSetupForm = document.querySelector('#totp-setup-form');
    if (totpSetupForm)
        totpSetupForm.onsubmit = event => { event.preventDefault(); confirmTOTPSetup(totpSetupForm); };
    ...
    if (elevationForm)
        elevationForm.onsubmit = async (event) => { event.preventDefault(); const password = String(new FormData(elevationForm).get('password') || ''), button = elevationForm.querySelector('button[type=submit]'), error = document.querySelector('#elevation-error'); button.disabled = true; button.textContent = '正在验证…'; error.hidden = true; try {
            await api('/api/v1/auth/elevate', { method: 'POST', body: jsonBody({ password }) });
            const pending = pendingElevation;
            ...
            error.hidden = false;
            button.disabled = false;
            button.textContent = '验证并继续';
            document.querySelector('#elevation-password')?.focus();
        } };
    ...
    document.querySelector('#modal-backdrop')?.addEventListener('click', e => { if (e.target.id === 'modal-backdrop')
        closeModal(); });
    ...
    document.querySelectorAll('[data-file-action]').forEach(button => button.onclick = async () => {
        const action = button.dataset.fileAction, item = state.modal?.item;
        if (!item)
            return;
        if (action === 'copy-path') {
            copyText(item.path, '路径已复制');
            return;
        }
        if (action === 'download') {
            await downloadManagedFile(item.path);
            return;
        }
        if (action === 'history') {
            loadFileBackups(item.path);
            return;
        }
        if (action === 'archive') {
            createArchive(item);
    ...
    document.querySelector('#file-editor')?.addEventListener('input', () => { if (state.modal)
...
async function closeModal() {
    if (state.modal?.dirty && !await askConfirm('当前编辑内容还没有保存。', { title: '放弃未保存的修改', confirmText: '放弃修改', danger: true }))
        return;
    if (state.modal?.kind === 'elevation' && pendingElevation) {
        const pending = pendingElevation;
        pendingElevation = null;
...
async function performLogout() {
    if (!await askConfirm('退出后需要重新输入密码登录。', { title: '退出当前账号', confirmText: '退出登录' }))
        return;
    try {
...
function render() {
    ...
    rememberRoute(location.pathname);
    app.innerHTML = shell((routes[location.pathname] || routes['/'])());
    bindShell();
    syncOverviewUpdates();
    syncDockerStats();
    syncSystemLogPolling();
}
// Feature composition and progressive enhancement.
function appendPageSection(html, section) { const index = html.lastIndexOf('</div>'); return index >= 0 ? html.slice(0, index) + section + html.slice(index) : html + section; }
...
} return appendPageSection(html, extra); };
const baseLoadFiles = loadFiles;
loadFiles = async function (path = '/') { await baseLoadFiles(path); try {
    state.filePreferences = await api('/api/v1/files/preferences');
}
catch { } if (location.pathname === '/files')
    render(); };
const baseLoadUpdates = loadUpdates;
loadUpdates = async function () { await baseLoadUpdates(); try {
    state.aptSources = await api('/api/v1/system/apt/sources');
}
catch { } if (location.pathname === '/updates')
    render(); };
function aptSourceView(source) {
    ...
    if (lower === 'deb.debian.org' || lower.endsWith('.debian.org')) {
        provider = 'Debian 官方源';
        kind = 'debian';
    }
    else if (lower.endsWith('.ubuntu.com')) {
        provider = 'Ubuntu 官方源';
        kind = 'ubuntu';
    }
    else if (lower.includes('docker.com')) {
        provider = 'Docker 官方源';
        kind = 'docker';
    }
    else if (lower.includes('microsoft.com')) {
        provider = 'Microsoft 软件源';
        kind = 'microsoft';
    }
    else if (lower.includes('nodesource.com')) {
        provider = 'NodeSource 软件源';
        kind = 'node';
    }
    else if (source?.name === 'sources.list') {
        provider = '系统主软件源';
        kind = 'system';
    }
...
async function registerPasskey() { if (!passkeyAvailable()) {
    ...
    await showError(error.name === 'NotAllowedError' ? 'Passkey 操作已取消或超时' : error.message);
} }
// Delegated interactions keep dynamically rendered pages maintainable.
document.addEventListener('submit', async (event) => {
    const form = event.target;
    if (form.id === 'docker-hub-search-form') {
        event.preventDefault();
        const query = String(new FormData(form).get('query') || '').trim();
        state.dockerHubQuery = query;
        try {
            state.dockerHubResults = await api(`/api/v1/docker/hub/search?q=${encodeURIComponent(query)}`);
            render();
        }
        catch (e) {
            await showError(e.message);
    ...
    if (form.id === 'audit-filter-form') {
        event.preventDefault();
        const f = new FormData(form);
        for (const key of ['q', 'user', 'ip', 'action', 'result', 'from', 'to'])
            state.auditQuery[key] = String(f.get(key) || '');
        state.auditQuery.offset = 0;
        await loadAudit();
    }
    if (form.id === 'github-assets-form') {
        event.preventDefault();
        const f = new FormData(form), data = state.github;
        try {
            state.githubAssets = await api(`/api/v1/github/release/assets?owner=${encodeURIComponent(data.owner)}&repo=${encodeURIComponent(data.name)}&tag=${encodeURIComponent(f.get('tag'))}`);
            render();
        }
        catch (e) {
            await showError(e.message);
    ...
    if (form.id === 'ip-allowlist-form') {
        event.preventDefault();
        const f = new FormData(form);
        try {
            const out = await secureApi('/api/v1/security/ip-allowlist', { method: 'POST', body: jsonBody({ enabled: f.get('enabled') === 'on', entries: lines(f.get('entries')) }) });
            if (out.recovery_url) {
                state.modal = { title: '紧急恢复链接', kind: 'logs', content: `请立即保存。15 分钟内访问可关闭允许列表：\n${location.origin}${out.recovery_url}` };
                render();
            }
            else {
                showToast('IP 允许列表已保存');
                await loadSecurity();
            }
        }
        catch (e) {
    ...
    if (form.id === 'login-notify-form') {
        event.preventDefault();
        const f = new FormData(form);
        try {
            await secureApi('/api/v1/security/login-notifications', { method: 'POST', body: jsonBody({ enabled: f.get('enabled') === 'on', bot_token: f.get('bot_token'), chat_id: f.get('chat_id'), test: f.get('test') === 'on' }) });
            showToast('登录通知已保存');
            await loadSecurity();
        }
        catch (e) {
    ...
    if (form.id === 'ufw-rule-form') {
        ...
        try {
            state.firewall = await secureApi('/api/v1/security/firewall/rule', {
                method: 'POST',
                body: jsonBody({
                    operation: 'add',
                    rule: {
...
document.addEventListener('click', async (event) => {
    const button = event.target.closest('button,[data-file-shortcut]');
    if (!button)
        return;
    if (button.dataset.hubPull) {
        const image = button.dataset.hubPull;
        const tag = await askText('填写镜像标签', { title: '拉取 Docker 镜像', value: image + ':latest' });
        if (tag) {
            try {
                await secureApi('/api/v1/docker/images/pull', { method: 'POST', body: jsonBody({ image: tag }) });
                showToast('镜像已拉取');
                await loadDocker();
            }
            catch (e) {
    ...
    if (button.id === 'refresh-volume-usage') {
        try {
            state.dockerVolumeUsage = await api('/api/v1/docker/volumes/usage');
            render();
        }
        catch (e) {
            await showError(e.message);
    ...
    if (button.dataset.fileShortcut) {
        const path = button.dataset.fileShortcut;
        if (button.dataset.dir === 'true')
            loadFiles(path);
        else
            openFile(path);
    }
    if (button.id === 'toggle-current-favorite') {
        try {
            state.filePreferences = await api('/api/v1/files/preferences', { method: 'POST', body: jsonBody({ action: button.dataset.enabled === 'true' ? 'unfavorite' : 'favorite', path: state.files?.path || '/', is_dir: true }) });
            render();
            showToast(button.dataset.enabled === 'true' ? '已取消收藏' : '已收藏目录');
        }
        catch (e) {
            await showError(e.message);
        }
    }
    if (button.id === 'add-apt-source') {
        const name = await askText('文件名会保存在 /etc/apt/sources.list.d/ 下', { title: '添加软件源', placeholder: '例如 custom.list' });
        if (!name)
            return;
        const content = await askText('输入完整 deb 行或 deb822 内容', { title: '软件源内容', placeholder: 'deb https://example.com/debian stable main' });
        if (!content)
            return;
        try {
    ...
    if (button.dataset.sourceToggle) {
        try {
            await secureApi('/api/v1/system/apt/sources', { method: 'POST', body: jsonBody({ action: button.dataset.enabled === 'true' ? 'disable' : 'enable', path: button.dataset.sourceToggle }) });
            showToast('软件源状态已更新');
            await loadUpdates();
        }
        catch (e) {
    ...
    if (button.dataset.sourceDelete) {
        if (await askConfirm('删除前会自动创建快照。', { title: '删除软件源', confirmText: '删除', danger: true }))
            try {
                await secureApi('/api/v1/system/apt/sources', { method: 'POST', body: jsonBody({ action: 'delete', path: button.dataset.sourceDelete }) });
                showToast('软件源已删除');
                await loadUpdates();
            }
            catch (e) {
    ...
    if (button.id === 'toggle-ntp') {
        try {
            await secureApi('/api/v1/system/host/ntp', { method: 'POST', body: jsonBody({ enabled: button.dataset.enabled !== 'true' }) });
            showToast('时间同步设置已更新');
            await loadHostSettings();
        }
        catch (e) {
    ...
    if (button.id === 'audit-prev') {
        state.auditQuery.offset = Math.max(0, state.auditQuery.offset - state.auditQuery.limit);
        await loadAudit();
    }
    if (button.id === 'audit-next') {
        state.auditQuery.offset += state.auditQuery.limit;
        await loadAudit();
    }
    if (button.hasAttribute('data-copy-audit')) {
        const item = (state.audit?.events || [])[Number(button.dataset.copyAudit)];
        if (item)
            await copyText(auditEventText(item), '审计记录已复制');
    }
    if (button.dataset.githubRun) {
        const data = state.github;
        try {
            state.githubJobs = await api(`/api/v1/github/actions/jobs?owner=${encodeURIComponent(data.owner)}&repo=${encodeURIComponent(data.name)}&run_id=${button.dataset.githubRun}`);
            render();
        }
        catch (e) {
            await showError(e.message);
    ...
    if (button.dataset.githubJob) {
        const data = state.github;
        try {
            const out = await api(`/api/v1/github/actions/job-logs?owner=${encodeURIComponent(data.owner)}&repo=${encodeURIComponent(data.name)}&job_id=${button.dataset.githubJob}`);
            state.modal = { title: 'GitHub Actions Job 日志', kind: 'logs', content: out.logs || '暂无日志' };
            render();
        }
        catch (e) {
            await showError(e.message);
    ...
    if (button.id === 'github-asset-upload') {
        const form = document.querySelector('#github-assets-form'), f = new FormData(form), file = form.querySelector('[name=file]').files?.[0], data = state.github;
        if (!file) {
            await showError('请选择要上传的附件');
            return;
        }
        const body = new FormData();
        body.append('owner', data.owner);
        body.append('repo', data.name);
        body.append('tag', f.get('tag'));
        body.append('file', file);
        try {
            await secureApi('/api/v1/github/release/assets/upload', { method: 'POST', body });
            showToast('Release 附件已上传');
            state.githubAssets = await api(`/api/v1/github/release/assets?owner=${encodeURIComponent(data.owner)}&repo=${encodeURIComponent(data.name)}&tag=${encodeURIComponent(f.get('tag'))}`);
            render();
        }
        catch (e) {
    ...
    if (button.id === 'register-passkey')
        registerPasskey();
    if (button.dataset.passkeyDelete && await askConfirm('删除后这把 Passkey 不能再登录。', { title: '移除 Passkey', confirmText: '移除', danger: true }))
        try {
            await secureApi('/api/v1/auth/passkeys', { method: 'DELETE', body: jsonBody({ id: button.dataset.passkeyDelete }) });
    ...
    if (button.dataset.trustedDelete)
        try {
            await secureApi('/api/v1/auth/trusted-devices', { method: 'DELETE', body: jsonBody({ id: button.dataset.trustedDelete }) });
            showToast('可信设备已撤销');
            await loadSecurity();
        }
        catch (e) {
    ...
    if (button.id === 'revoke-all-trusted' && await askConfirm('所有设备下次登录都需要验证码。', { title: '撤销全部可信设备', confirmText: '全部撤销', danger: true }))
        try {
            await secureApi('/api/v1/auth/trusted-devices', { method: 'DELETE', body: jsonBody({ all: true }) });
            showToast('全部可信设备已撤销');
            await loadSecurity();
        }
        catch (e) {
    ...
    if (button.id === 'install-ufw')
        try {
            await secureApi('/api/v1/security/firewall/install', { method: 'POST', body: '{}' });
            showToast('UFW 已安装');
    ...
    if (button.id === 'enable-ufw' && await askConfirm('面板会保留 SSH/current IP，并设置 5 分钟自动关闭保护。启用后请立即确认当前连接正常。', { title: '安全启用 UFW', confirmText: '启用' }))
        try {
            state.firewall = await secureApi('/api/v1/security/firewall/enable', { method: 'POST', body: '{}' });
            showToast('UFW 已启用，请在 5 分钟内确认');
            render();
        }
        catch (e) {
            await showError(e.message);
        }
    if (button.id === 'confirm-ufw')
        try {
            state.firewall = await secureApi('/api/v1/security/firewall/confirm', { method: 'POST', body: '{}' });
            showToast('连接已确认，自动恢复任务已取消');
            render();
        }
        catch (e) {
            await showError(e.message);
        }
    if (button.id === 'disable-ufw' && await askConfirm('关闭后所有端口将不再由 UFW 过滤。', { title: '关闭 UFW', confirmText: '关闭', danger: true }))
        try {
            state.firewall = await secureApi('/api/v1/security/firewall/disable', { method: 'POST', body: '{}' });
            showToast('UFW 已关闭');
            render();
        }
        catch (e) {
...
const dockerPageWithFeatures = dockerPage;
dockerPage = function () { let html = dockerPageWithFeatures(); if (state.dockerTab === 'containers') {
    html = html.replace(/<button class="secondary-button compact" data-docker-edit="([^"]+)" data-title="([^"]+)">编辑<\/button>/g, (match, id, title) => match + `<button class="secondary-button compact" data-docker-diagnostic="${id}" data-title="${title}">诊断</button>`);
} if (state.dockerTab === 'volumes') {
    html = html.replace(/<button class="danger-button compact" data-volume-delete="([^"]+)">删除<\/button>/g, (match, name) => `<button class="secondary-button compact" data-volume-backup="${name}">备份</button><button class="secondary-button compact" data-volume-restore="${name}">恢复</button>${match}`);
} return html; };
document.addEventListener('click', async (event) => {
    const button = event.target.closest('button');
    if (!button)
        return;
    if (button.dataset.dockerDiagnostic) {
        const options = [['identity', '身份与权限'], ['working-directory', '当前工作目录'], ['environment', '环境变量'], ['disk', '容器磁盘'], ['processes', '进程列表'], ['network', '网络统计'], ['os-release', '系统版本'], ['list-root', '根目录列表']], choice = await chooseAction(`${button.dataset.title} · 安全诊断`, options.map(([, label]) => ({ label, description: '执行固定命令，不支持任意 Shell' })));
        if (choice === false || choice === null)
            return;
        try {
            const out = await secureApi('/api/v1/docker/exec', { method: 'POST', body: jsonBody({ id: button.dataset.dockerDiagnostic, command: options[choice][0] }) });
            state.modal = { title: `${button.dataset.title} · ${options[choice][1]}`, kind: 'logs', content: out.output || '命令完成但没有输出' };
            render();
        }
        catch (e) {
            await showError(e.message);
    ...
    if (button.dataset.volumeBackup) {
        const path = await askText('备份会保存为 tar.gz，请填写完整的文件系统路径。', { title: `备份卷 ${button.dataset.volumeBackup}`, value: `/opt/${button.dataset.volumeBackup}-${new Date().toISOString().slice(0, 10)}.tar.gz` });
        if (!path)
            return;
        try {
            const out = await secureApi('/api/v1/docker/volumes/archive', { method: 'POST', body: jsonBody({ action: 'backup', name: button.dataset.volumeBackup, path }) });
            showToast(`备份已保存：${out.path}`);
        }
        catch (e) {
            await showError(e.message);
        }
    }
    if (button.dataset.volumeRestore) {
        const path = await askText('恢复是覆盖/追加模式，不会删除备份中不存在的旧文件。建议先备份当前卷。', { title: `恢复卷 ${button.dataset.volumeRestore}`, placeholder: '/opt/volume-backup.tar.gz' });
        if (!path)
            return;
        if (!await askConfirm('恢复会覆盖卷内同名文件。容器运行中时可能产生不一致，建议先停止相关容器。', { title: '确认恢复存储卷', confirmText: '开始恢复', danger: true }))
            return;
        try {
            await secureApi('/api/v1/docker/volumes/archive', { method: 'POST', body: jsonBody({ action: 'restore', name: button.dataset.volumeRestore, path }) });
            showToast('存储卷已恢复');
        }
        catch (e) {
            await showError(e.message);
...
document.addEventListener('click', event => { const button = event.target.closest('#preview-markdown'); if (!button)
    return; const content = document.querySelector('#file-editor')?.value || ''; state.modal = { title: `${state.modal?.title || 'Markdown'} · 预览`, kind: 'markdown-preview', content, path: state.modal?.path, originalTitle: state.modal?.title }; render(); });
const baseFeatureModalHTML = modalHTML;
modalHTML = function () { if (state.modal?.kind === 'markdown-preview') {
    const m = state.modal;
    return `<div class="modal-backdrop" id="modal-backdrop"><section class="modal-card markdown-modal"><header><h2>${escapeHTML(m.title)}</h2><button id="modal-close" aria-label="关闭">${icon('close', 20)}</button></header><div class="modal-body markdown-preview">${renderMarkdownSafe(m.content)}</div><footer><button id="markdown-back-edit" class="secondary-button">返回编辑</button><button id="modal-done" class="primary-button">完成</button></footer></section></div>`;
} return baseFeatureModalHTML(); };
document.addEventListener('click', event => { if (!event.target.closest('#markdown-back-edit'))
    return; const m = state.modal; state.modal = { title: m.originalTitle || 'Markdown', kind: 'editor', content: m.content, path: m.path, dirty: false, preview_kind: 'markdown' }; render(); });
document.addEventListener('click', async (event) => {
    const button = event.target.closest('button');
    if (!button)
        return;
    if (button.id === 'create-linux-user') {
        const name = await askText('新用户会创建 Home 目录，并锁定密码。创建后请为它生成或添加 SSH 公钥。', { title: '新建 Linux 用户', placeholder: '例如 deploy' });
        if (!name)
            return;
        const sudo = await askConfirm('是否同时授予 sudo 管理权限？', { title: '用户权限', confirmText: '授予 sudo', cancelText: '普通用户' });
        try {
            await secureApi('/api/v1/ssh/users/manage', { method: 'POST', body: jsonBody({ action: 'create', name, sudo: !!sudo }) });
            showToast('Linux 用户已创建');
            await loadSSH(name);
        }
        catch (e) {
            await showError(e.message);
    ...
    if (button.dataset.userSudo) {
        const enabled = button.dataset.enabled !== 'true';
        if (!await askConfirm(enabled ? '授予后这个用户可以通过 sudo 获得 root 权限。' : '移除后该用户将不能再使用 sudo。', { title: enabled ? '授予 sudo' : '移除 sudo', confirmText: '确认' }))
            return;
        try {
            await secureApi('/api/v1/ssh/users/manage', { method: 'POST', body: jsonBody({ action: 'sudo', name: button.dataset.userSudo, sudo: enabled }) });
            showToast('用户权限已更新');
            await loadSSH(state.sshUser);
        }
        catch (e) {
    ...
    if (button.dataset.userDelete) {
        if (!await askConfirm('系统会拒绝删除 root、服务用户和仍有运行进程的用户。是否同时删除 Home 目录？', { title: `删除用户 ${button.dataset.userDelete}`, confirmText: '删除用户和 Home', danger: true }))
            return;
        try {
            await secureApi('/api/v1/ssh/users/manage', { method: 'POST', body: jsonBody({ action: 'delete', name: button.dataset.userDelete, remove_home: true }) });
            showToast('Linux 用户已删除');
            state.sshUser = '';
            await loadSSH();
        }
        catch (e) {
...
document.addEventListener('click', async (event) => { const button = event.target.closest('[data-log-source]'); if (!button)
    return; state.logSource = button.dataset.logSource || ''; await loadAudit(); });
const updatesPageWithJobs = updatesPage;
updatesPage = function () { return appendPageSection(updatesPageWithJobs(), backgroundJobPanel('apt.')); };
const dockerPageWithJobs = dockerPage;
dockerPage = function () { return appendPageSection(dockerPageWithJobs(), backgroundJobPanel('docker.')); };
const modalWithJobs = modalHTML;
modalHTML = function () {
    ...
    return modalWithJobs();
};
document.addEventListener('click', async (event) => {
    const button = event.target.closest('button');
    if (!button)
        return;
    if (button.id === 'refresh-background-jobs')
        await loadBackgroundJobs();
    if (button.dataset.backgroundJob) {
        try {
            const out = await api(`/api/v1/jobs?id=${encodeURIComponent(button.dataset.backgroundJob)}`), job = out.job;
            state.modal = job.status === 'running' || job.status === 'queued' ? { title: job.kind, kind: 'job-progress', job } : { title: job.kind, kind: 'logs', content: jobResultOutput(job) || job.error || '任务没有输出' };
            render();
        }
        catch (error) {
            await showError(error.message);
...
document.addEventListener('click', event => { const button = event.target.closest('[data-github-pr-merge]'); if (button)
    mergeGitHubPullRequest(button); });
const snapshotsPageWithScheduled = snapshotsPage;
snapshotsPage = function () {
    let html = snapshotsPageWithScheduled();
    ...
    return appendPageSection(html, panel);
};
async function downloadScheduledBackup(name) {
    ...
    if (response.status === 403) {
        await requestElevation();
        response = await fetch(`/api/v1/backup/scheduled?download=${encodeURIComponent(name)}`, { credentials: 'same-origin' });
    }
    if (!response.ok) {
        const body = await response.json().catch(() => ({}));
...
document.addEventListener('click', async (event) => {
    ...
    if (button.id === 'create-scheduled-backup-now') {
        try {
            const out = await secureApi('/api/v1/backup/scheduled', { method: 'POST', body: jsonBody({ action: 'create' }) });
            state.scheduledBackups = out;
            showToast('完整备份已生成');
            render();
        }
        catch (e) {
    ...
    if (button.dataset.scheduledBackupDownload) {
        try {
            await downloadScheduledBackup(button.dataset.scheduledBackupDownload);
        }
        catch (e) {
            await showError(e.message);
        }
    ...
    if (button.dataset.scheduledBackupDelete) {
        if (!await askConfirm('删除后无法恢复。', { title: '删除定时备份', confirmText: '删除', danger: true }))
            return;
        try {
            const out = await secureApi('/api/v1/backup/scheduled', { method: 'POST', body: jsonBody({ action: 'delete', name: button.dataset.scheduledBackupDelete }) });
            state.scheduledBackups = out;
            showToast('定时备份已删除');
            render();
        }
...
function stopSystemLogPolling() { if (state.systemLogTimer) {
    clearInterval(state.systemLogTimer);
...
async function refreshSystemLogsOnly() {
    ...
    try {
        ...
        if (pre) {
            pre.textContent = state.systemLogs?.logs || '暂无系统日志';
            if (nearBottom)
...
function stopDockerLogPolling() { if (state.dockerLogTimer) {
    clearInterval(state.dockerLogTimer);
    state.dockerLogTimer = null;
} }
async function refreshDockerLogs(id) {
    if (!state.modal || state.modal.dockerLogID !== id) {
        stopDockerLogPolling();
        return;
    }
    if (state.dockerLogPaused || document.hidden)
        return;
    try {
        ...
        if (pre) {
            pre.textContent = state.modal.content;
            if (nearBottom)
                pre.scrollTop = pre.scrollHeight;
        }
        else
        ...
        if (state.modal?.dockerLogID === id) {
            state.modal.content = `日志读取失败：${e.message}`;
            const pre = document.querySelector('.modal-log');
            if (pre)
                pre.textContent = state.modal.content;
        }
    }
}
const modalWithLiveDockerLogs = modalHTML;
modalHTML = function () {
    let html = modalWithLiveDockerLogs();
...
document.addEventListener('click', async (event) => {
    ...
    if (!button)
        return;
    if (button.id === 'install-fail2ban') {
        if (!await askConfirm('会安装 Fail2ban、保护 SSH，并自动加入当前 IP、内网和现有 SSH 会话来源地址。', { title: '安全安装 Fail2ban', confirmText: '安装' }))
            return;
        try {
            await secureApi('/api/v1/security/fail2ban/install', { method: 'POST', body: '{}' });
            showToast('Fail2ban 已安装并启用');
            await loadSecurity();
        }
        catch (e) {
            await showError(e.message);
    ...
    if (button.dataset.fail2banUnban) {
        if (!await askConfirm(`解除 ${button.dataset.fail2banUnban} 的 SSH 封禁？`, { title: '解除封禁', confirmText: '解封' }))
            return;
        try {
            await secureApi('/api/v1/security/fail2ban/unban', { method: 'POST', body: jsonBody({ ip: button.dataset.fail2banUnban }) });
            showToast('地址已解封');
            await loadSecurity();
        }
        catch (e) {
    ...
    if (button.dataset.fail2banIgnoreRemove) {
        if (!await askConfirm(`从 SSH 白名单移除 ${button.dataset.fail2banIgnoreRemove}？当前访问 IP 不允许移除。`, { title: '移除白名单', confirmText: '移除', danger: true }))
            return;
        try {
            await secureApi('/api/v1/security/fail2ban/ignore', { method: 'POST', body: jsonBody({ entry: button.dataset.fail2banIgnoreRemove, action: 'remove' }) });
            showToast('白名单已更新');
            await loadSecurity();
        }
        catch (e) {
...
/* GitHub Device Flow and keyboard-safe managed-file actions. */
async function downloadManagedFile(path) {
    ...
    if (response.status === 403) {
        ...
        if (String(body.error || '').includes('二次验证')) {
            await requestElevation();
    ...
    link.href = url;
    link.download = encoded ? decodeURIComponent(encoded) : plain || String(path).split('/').pop() || 'download';
    document.body.appendChild(link);
    link.click();
    link.remove();
    setTimeout(() => URL.revokeObjectURL(url), 2000);
...
function requestElevation() {
    ...
    if (pendingElevation) {
        pendingElevation.reject(new Error('二次验证已被新的请求替代'));
        pendingElevation = null;
    }
    return new Promise((resolve, reject) => {
        const root = document.createElement('div');
        root.className = 'elevation-dialog-backdrop';
        root.dataset.elevationDialog = 'true';
        ...
        const finish = (ok, value) => { if (root.isConnected)
            root.remove(); document.body.classList.remove('elevation-open'); syncOverlayState(); pendingElevation = null; ok ? resolve(value) : reject(value instanceof Error ? value : new Error('操作已取消')); };
        ...
        root.onclick = event => { if (event.target === root)
            finish(false, new Error('操作已取消')); };
        form.onsubmit = async (event) => { event.preventDefault(); const password = input.value; if (!password) {
            focusInput();
            return;
        } submit.disabled = true; submit.textContent = '正在验证…'; error.hidden = true; try {
            await api('/api/v1/auth/elevate', { method: 'POST', body: jsonBody({ password }) });
            finish(true, true);
        }
        catch (err) {
        ...
        document.body.appendChild(root);
        document.body.classList.add('elevation-open');
        syncOverlayState();
        syncVisualViewport();
        requestAnimationFrame(() => { root.classList.add('show'); focusInput(); });
    });
}
function finishPasswordLogin(result, username) {
    localStorage.setItem('lukepanel-login-user', String(username || ''));
    state.authenticated = true;
    state.username = result.username;
...
function renderOTPLogin(challenge) {
    stopOverviewUpdates();
    ...
    trust.onchange = () => name.hidden = !trust.checked;
    document.querySelector('#otp-login-back').onclick = () => { state.loginChallenge = null; renderLogin(); };
    form.onsubmit = async (event) => { event.preventDefault(); button.disabled = true; button.textContent = '正在验证…'; error.hidden = true; const f = new FormData(form); try {
        const result = await api('/api/v1/auth/login', { method: 'POST', body: jsonBody({ username: challenge.username, password: challenge.password, otp: f.get('otp'), trust_device: f.get('trust_device') === 'on', device_name: f.get('device_name') }) });
        await finishPasswordLogin(result, challenge.username);
    }
    catch (err) {
        error.textContent = err.message;
        error.hidden = false;
...
function renderLogin() {
    ...
    document.querySelector('#passkey-login')?.addEventListener('click', async () => { const button = document.querySelector('#passkey-login'), username = String(form.querySelector('[name=username]')?.value || '').trim(); error.hidden = true; button.disabled = true; button.textContent = '正在调用 Passkey…'; try {
        const begin = await api('/api/v1/auth/passkey/login/begin', { method: 'POST', body: jsonBody({ username }) });
        const credential = await navigator.credentials.get({ publicKey: { challenge: b64urlToBytes(begin.challenge), rpId: begin.rp_id, timeout: begin.timeout, userVerification: begin.user_verification, allowCredentials: (begin.allow_credentials || []).map(item => ({ ...item, id: b64urlToBytes(item.id) })) } });
        const result = await api('/api/v1/auth/passkey/login/finish', { method: 'POST', body: jsonBody({ flow_id: begin.flow_id, credential: serializeAssertion(credential) }) });
        await finishPasswordLogin(result, username);
    }
    catch (err) {
        error.textContent = err.name === 'NotAllowedError' ? 'Passkey 操作已取消或超时' : err.message;
        error.hidden = false;
    }
    finally {
        button.disabled = false;
        button.innerHTML = `${icon('key', 18)}使用 Passkey 登录`;
    } });
    form.onsubmit = async (event) => { event.preventDefault(); const f = new FormData(form), username = String(f.get('username') || '').trim(), password = String(f.get('password') || ''), button = form.querySelector('button[type=submit]'); button.disabled = true; button.textContent = '正在登录…'; error.hidden = true; try {
        const result = await api('/api/v1/auth/login', { method: 'POST', body: jsonBody({ username, password }) });
        await finishPasswordLogin(result, username);
    }
    catch (err) {
        if (err.code === 'totp_required') {
            renderOTPLogin({ username, password });
            return;
        }
        error.textContent = err.message;
        error.hidden = false;
...
async function cancelGitHubDeviceFlow() {
    if (state.githubFlowTimer) {
        clearTimeout(state.githubFlowTimer);
        state.githubFlowTimer = null;
    }
    const flowID = state.githubFlow?.flow_id || '';
    state.githubFlow = null;
    try {
        await api('/api/v1/github/auth/device/cancel', { method: 'POST', body: jsonBody({ flow_id: flowID }) });
    }
    catch { }
    render();
}
async function loginWithGitHubToken(form) {
    const token = String(new FormData(form).get('token') || '').trim(), button = form.querySelector('button');
    button.disabled = true;
    button.textContent = '正在连接…';
    try {
        const result = await api('/api/v1/github/auth/token', { method: 'POST', body: jsonBody({ token }) });
        state.githubAuth = { connected: true, ...result };
        form.reset();
        showToast(`已连接 GitHub @${result.login}`);
        render();
...
document.addEventListener('submit', event => { if (event.target.id === 'github-token-form') {
    event.preventDefault();
    loginWithGitHubToken(event.target);
} });
document.addEventListener('click', event => { if (event.target.closest('#cancel-github-device'))
    cancelGitHubDeviceFlow(); });
const baseUpdatePasswordStrength = updatePasswordStrength;
updatePasswordStrength = function () {
    ...
    if (checks)
        checks.hidden = !input?.value;
    baseUpdatePasswordStrength();
};
...
/* Product UI composition: responsive layout, real status, mobile-first interaction. */
function percentOf(value, total) { return total ? clamp(Number(value || 0) / Number(total) * 100) : 0; }
function overviewStatCard(id, label, value, detail, iconName, tone = 'blue') {
    return `<article class="overview-stat-card surface tone-${tone}" data-kpi="${id}"><div class="overview-stat-icon">${icon(iconName, 20)}</div><div><span>${escapeHTML(label)}</span><strong data-kpi-value>${escapeHTML(value)}</strong><small data-kpi-detail>${escapeHTML(detail || '')}</small></div></article>`;
}
function resourceMeterRow(id, label, value, detail, percent, tone = 'blue') {
    return `<div class="resource-meter-row" data-resource="${id}"><div><span>${escapeHTML(label)}</span><strong data-resource-value>${escapeHTML(value)}</strong></div><div class="resource-meter-track"><i class="tone-${tone}" data-resource-progress style="width:${clamp(percent)}%"></i></div><small data-resource-detail>${escapeHTML(detail || '')}</small></div>`;
}
function infoListRow(label, value) { return `<div><dt>${escapeHTML(label)}</dt><dd>${escapeHTML(value || '-')}</dd></div>`; }
function sysctlPresetOption(id, label, description, current, danger = false) { return `<button type="button" data-sysctl-preset="${id}" class="profile-option ${current === id ? 'active' : ''} ${danger ? 'danger' : ''}"><span>${current === id ? icon('check', 17) : icon(id === 'network' ? 'network' : id === 'low-memory' ? 'drive' : 'activity', 17)}</span><div><strong>${escapeHTML(label)}</strong><small>${escapeHTML(description)}</small></div>${current === id ? '<em>当前</em>' : ''}</button>`; }
function auditResultBadge(result) { const success = result === 'success', failed = ['failed', 'error'].includes(result); return `<span class="audit-result-badge ${success ? 'success' : failed ? 'danger' : 'muted'}"><i></i>${success ? '成功' : failed ? '失败' : escapeHTML(result || '未知')}</span>`; }
restore();

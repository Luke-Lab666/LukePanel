...
function githubAuthCard(){
  ...
  if(auth.connected)return `<section class="surface github-auth-card connected"><div class="github-user">${auth.avatar_url?`<img src="${escapeHTML(auth.avatar_url)}" alt="">`:icon('github',24)}<div><strong>已连接 @${escapeHTML(auth.login)}</strong><span>授权仅保存在当前 LukePanel 会话内</span></div></div><button id="github-disconnect" class="secondary-button compact">断开连接</button></section>`
  const deviceButton=auth.device_login_available?`<form id="github-connect-form"><button class="primary-button" type="submit">${icon('github',18)}GitHub 设备登录</button></form>`:''
  return `<section class="surface github-auth-card"><div><h2>连接 GitHub</h2></div>${deviceButton}${state.githubFlow?`<div class="device-flow-box"><span>授权码</span><strong>${escapeHTML(state.githubFlow.user_code)}</strong><div><button id="copy-github-code" class="secondary-button compact">复制代码</button><a class="primary-button compact" href="${escapeHTML(state.githubFlow.verification_uri)}" target="_blank" rel="noopener">打开 GitHub</a><button id="cancel-github-device" class="secondary-button compact">取消</button></div></div>`:''}<details class="token-guide"><summary>使用 Token 登录</summary><form id="github-token-form" class="github-token-form"><label>Fine-grained Token<input name="token" type="password" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" required></label><button class="secondary-button" type="submit">连接</button></form></details></section>`
}
...
function githubPage(){
  ...
  const latest=data?.latest_release,tagSuggestion=latest?.tag_name?nextVersionSuggestion(latest.tag_name):'v0.9.6-beta'
...
function nextVersionSuggestion(current){const match=String(current).match(/^v(\d+)\.(\d+)\.(\d+)(.*)$/);if(!match)return 'v0.9.6-beta';const major=Number(match[1]),minor=Number(match[2]),patch=Number(match[3]),suffix=match[4]||'';if(major===0&&minor<9)return 'v0.9.6-beta';if(major===0&&minor===9)return `v0.9.${patch+1}${suffix||'-beta'}`;return `v${major}.${minor+1}.0`}

async function startGitHubDeviceFlow(form){
  const button=form.querySelector('button'),popup=window.open('about:blank','_blank')
  button.disabled=true;button.textContent='正在创建登录…'
  try{state.githubFlow=await api('/api/v1/github/auth/device/start',{method:'POST',body:'{}'});if(popup)popup.location.href=state.githubFlow.verification_uri;render();scheduleGitHubPoll()}catch(error){popup?.close();await showError(error.message)}finally{if(button.isConnected){button.disabled=false;button.textContent='GitHub 设备登录'}}
}
...
/* v0.9.6 mobile authentication, full-filesystem and GitHub device login fixes */

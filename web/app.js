...
function systemPage(){
  const modules=[
    ...
    ['进程管理','CPU、内存排行与进程结束','/processes','activity',true],['网络管理','接口、流量与监听端口','/network','network',true],['存储管理','分区、文件系统与空间','/storage','drive',true],['计划任务','systemd timer 查看','/tasks','calendar',true],['软件更新','APT 可升级软件包检查','/updates','package',true],['SSH 管理','登录状态与公钥管理','/ssh','key',true]
  ]
...
function githubPage(){
  ...
  const latest=data?.latest_release,tagSuggestion=latest?.tag_name?nextVersionSuggestion(latest.tag_name):'v0.7.0-alpha'
...
function nextVersionSuggestion(current){const match=String(current).match(/^v(\d+)\.(\d+)\.(\d+)(.*)$/);if(!match)return 'v0.7.0-alpha';return `v${match[1]}.${Number(match[2])+1}.0-alpha`}
...
function modalHTML(){
  ...
  return `<div class="modal-backdrop" id="modal-backdrop"><section class="modal-card ${['editor','logs','docker-edit','file-diff','file-backups','docker-cleanup','task-create','ssh-generated-key'].includes(m.kind)?'wide':''}"><header><div><strong>${escapeHTML(m.title)}</strong>${m.path?`<small>${escapeHTML(m.path)}</small>`:''}</div><button id="modal-close">${icon('close',20)}</button></header><div class="modal-body">${body}</div>${footer}</section></div>`
}

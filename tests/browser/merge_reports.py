...
report={'version':'v2.0.5','framework':'React 18.2.0','render_checks':len(rows),'render_passed':sum(1 for r in rows if r['passed']),'interaction_checks':interaction_checks,'interaction_passed':interaction_passed,'failures':failures,'interactions':interaction_tests,'results':rows,'duration_seconds':round(duration,2)}

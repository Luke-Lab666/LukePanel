...
LukePanel v2.0.7 在 v2.0.6 基础上收紧敏感读取、恢复令牌、登录限速和 WebAuthn 信任边界。即使如此，也不要在没有 HTTPS、强密码、访问控制和可信反向代理的情况下暴露到公网。
...
- 文件访问经过路径规范化、符号链接解析和虚拟文件系统写保护；任何文件内容、预览、下载和历史版本读取均要求近期二次验证。
...
- 所有文件内容读取均需近期二次验证；写入、删除、上传、权限调整等高风险操作同样需要二次验证
- 在线编辑和 SSH 公钥修改前自动备份
- 文件删除先进入隔离回收站
...
- 面板 IP 允许列表保存时保留当前地址，并生成独立恢复页面与限时一次性令牌；令牌不进入 URL。

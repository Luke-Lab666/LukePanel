...
LukePanel v2.0.2 已进入稳定维护阶段。即使如此，也不要在没有 HTTPS、强密码、访问控制和可信反向代理的情况下暴露到公网。
...
- 支持 TOTP、一次性恢复码和 Passkey / WebAuthn。
- 账户密码登录在启用 TOTP 后始终要求验证码或恢复码，不接受可信设备 Cookie 绕过。
- Passkey 登录不要求先输入用户名，并通过浏览器凭据选择完成独立强认证；用户验证级别为 `required`。
- Passkey 挑战短时有效，凭据只保存公钥和计数器，不保存生物信息。
- 修改用户名和密码时，在同一表单验证当前密码与第二因素；其他高风险操作要求短时有效的管理员密码，启用 TOTP 时还必须验证第二因素。

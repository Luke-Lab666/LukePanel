...
func AddUFWRule(ctx context.Context, request FirewallRuleRequest) (FirewallStatus, error) {
	...
	normalized, err := normalizeUFWRuleRequest(request)
	if err != nil {
		return FirewallStatus{}, err
	}
	args := buildUFWRuleArgs(normalized)
...
func normalizeUFWRuleRequest(request FirewallRuleRequest) (FirewallRuleRequest, error) {
	...
	if request.Action != "allow" && request.Action != "deny" && request.Action != "reject" && request.Action != "limit" {
		return FirewallRuleRequest{}, errors.New("规则动作无效")
	}
	if request.Direction != "in" && request.Direction != "out" {
		return FirewallRuleRequest{}, errors.New("规则方向无效")
	}
	if request.Protocol == "" {
		request.Protocol = "tcp"
	}
	if request.Protocol != "tcp" && request.Protocol != "udp" && request.Protocol != "any" {
		return FirewallRuleRequest{}, errors.New("协议只支持 TCP、UDP 或任意")
	}
	if !firewallPortPattern.MatchString(request.Port) {
		return FirewallRuleRequest{}, errors.New("端口必须是单个端口或端口范围")
	}
	portParts := strings.Split(request.Port, ":")
	for _, p := range portParts {
		n, _ := strconv.Atoi(p)
		if n < 1 || n > 65535 {
			return FirewallRuleRequest{}, errors.New("端口必须是 1-65535")
		}
	}
	if len(portParts) == 2 {
		...
		if start > end {
			return FirewallRuleRequest{}, errors.New("端口范围起始值不能大于结束值")
		}
		if request.Protocol == "any" {
			return FirewallRuleRequest{}, errors.New("端口范围必须选择 TCP 或 UDP")
		}
	}
	if request.Action == "limit" && request.Protocol != "tcp" {
		return FirewallRuleRequest{}, errors.New("连接限速仅支持 TCP")
	}
	if request.Source != "" && request.Source != "any" {
		if _, _, err := net.ParseCIDR(request.Source); err != nil && net.ParseIP(request.Source) == nil {
			return FirewallRuleRequest{}, errors.New("来源必须是有效 IP 或 CIDR")
		}
	}
	if request.Comment != "" {
		if len(request.Comment) > 80 || strings.ContainsAny(request.Comment, "\x00\r\n") {
			return FirewallRuleRequest{}, errors.New("备注无效")
		}
	}
	return request, nil
}

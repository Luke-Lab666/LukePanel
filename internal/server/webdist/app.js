...
function renderLogin(){
  stopOverviewUpdates()
  app.innerHTML=`<main class="login-page"><section class="login-card surface"><div class="login-logo">L</div><h1>登录 LukePanel</h1><p>安全、轻量地管理你的服务器</p><form id="login-form"><label>用户名<input name="username" value="admin" autocomplete="username"></label><label>密码<div class="password-field"><input name="password" type="password" autocomplete="current-password" inputmode="latin" lang="en" autocapitalize="none" autocorrect="off" spellcheck="false" autofocus><button type="button" id="show-password">显示</button></div></label><label id="otp-field" hidden>验证码或恢复码<input name="otp" autocomplete="one-time-code" inputmode="text" lang="en" autocapitalize="characters" autocorrect="off" spellcheck="false" placeholder="6 位验证码或 XXXX-XXXX-XXXX"></label><div id="login-error" class="form-error" hidden></div><button class="primary-button" type="submit">登录</button></form><div class="security-note">${icon('shield',17)}会话仅通过 HttpOnly Cookie 保存</div></section></main>`
  ...
  document.querySelector('#login-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),button=e.currentTarget.querySelector('button[type=submit]'),error=document.querySelector('#login-error'),otpField=document.querySelector('#otp-field');button.disabled=true;button.textContent='正在登录…';error.hidden=true;try{const r=await api('/api/v1/auth/login',{method:'POST',body:jsonBody({username:f.get('username'),password:f.get('password'),otp:f.get('otp')})});state.authenticated=true;state.username=r.username;state.csrf=r.csrf_token;state.settings=await api('/api/v1/settings');navigate(rememberedRoute('/'),{replace:true})}catch(err){if(err.code==='totp_required'){otpField.hidden=false;otpField.querySelector('input').focus()}error.textContent=err.message;error.hidden=false}finally{button.disabled=false;button.textContent='登录'}}
}
...
function githubPage(){
  ...
  const latest=data?.latest_release,tagSuggestion=latest?.tag_name?nextVersionSuggestion(latest.tag_name):'v0.6.0-alpha'
...
async function pollGitHubDeviceFlow(){if(!state.githubFlow?.flow_id)return;try{const result=await api('/api/v1/github/auth/device/poll',{method:'POST',body:jsonBody({flow_id:state.githubFlow.flow_id})});if(result.status==='authorized'){state.githubFlow=null;state.githubAuth={connected:true,...result};showToast(`已连接 GitHub @${result.login}`);await loadGitHub(githubDefaults().owner,githubDefaults().repo);return}if(['expired','denied'].includes(result.status)){state.githubFlow=null;await showError(result.message||'GitHub 登录已取消或过期');render();return}state.githubFlow.interval=result.retry_after||state.githubFlow.interval;scheduleGitHubPoll()}catch(error){state.githubFlow=null;await showError(error.message);render()}}
...
function modalHTML(){
  ...
  return `<div class="modal-backdrop" id="modal-backdrop"><section class="modal-card ${['editor','logs','docker-edit','file-diff','file-backups','docker-cleanup','task-create'].includes(m.kind)?'wide':''}"><header><div><strong>${escapeHTML(m.title)}</strong>${m.path?`<small>${escapeHTML(m.path)}</small>`:''}</div><button id="modal-close">${icon('close',20)}</button></header><div class="modal-body">${body}</div>${footer}</section></div>`
}

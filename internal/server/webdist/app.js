...
function githubPage() {
    ...
    const latest = data?.latest_release, tagSuggestion = latest?.tag_name ? nextVersionSuggestion(latest.tag_name) : 'v1.0.0';
...
document.addEventListener('click', async (event) => {
    ...
    if (button.id === 'enable-ufw' && await askConfirm('面板会保留 SSH/current IP，并设置 5 分钟自动关闭保护。启用后请立即确认当前连接正常。', { title: '安全启用 UFW', confirmText: '启用' }))
        try {
            await secureApi('/api/v1/security/firewall/enable', { method: 'POST', body: '{}' });
    ...
    if (button.id === 'confirm-ufw')
        try {
            await secureApi('/api/v1/security/firewall/confirm', { method: 'POST', body: '{}' });
    ...
    if (button.id === 'disable-ufw' && await askConfirm('关闭后所有端口将不再由 UFW 过滤。', { title: '关闭 UFW', confirmText: '关闭', danger: true }))
        try {
            await secureApi('/api/v1/security/firewall/disable', { method: 'POST', body: '{}' });
            showToast('UFW 已关闭');
            await loadSecurity();
        }
        catch (e) {

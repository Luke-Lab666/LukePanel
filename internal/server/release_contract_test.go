...
func TestStableReleaseMetadataIsConsistent(t *testing.T) {
	...
	checks := map[string][]string{
		filepath.Join(root, "VERSION"):                             {"v2.0.0"},
		filepath.Join(root, "README.md"):                           {"当前版本：<code>v2.0.0</code>", "完整 UI 与防火墙可靠性重构版本"},
		filepath.Join(root, "SECURITY.md"):                         {"v2.0.0 已进入稳定维护阶段"},
		filepath.Join(root, ".github", "workflows", "release.yml"): {"版本号，例如 v2.0.0"},
	}
